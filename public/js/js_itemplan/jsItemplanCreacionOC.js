...
function getPtrByHojaGestion(component) {
    ...
    $.ajax({
        type: 'POST',
        url: 'getPtrByHGestionOCItemPlan',
        data: {idhg: idhg,
            hg: hg,
            ts: ts}
    }).done(function (data) {
...
function btnPxq(component) {
    console.log($(component).attr('data-SubProyecto'));
    console.log($(component).attr('data-EmpresaColab'));
    console.log($(component).attr('data-Jefatura'));
    console.log($(component).attr('data-paquetizado_fg'));
    console.log($(component).attr('data-itemPlan'));

    $.ajax({
        type: 'POST',
        'url': 'getListarPxq',
        data: {SubProyecto: $(component).attr('data-SubProyecto'),
            EmpresaColab: $(component).attr('data-EmpresaColab'),
            Jefatura: $(component).attr('data-Jefatura'),
            paquetizado_fg: $(component).attr('data-paquetizado_fg'),
            itemPlan: $(component).attr('data-itemPlan')},
        'async': false
    }).done(function (data) {
        data = JSON.parse(data);
        if (data.error == 0) {
            $('#modaPxq').modal('toggle');
            $('#contTablaPxq').html(data.tablaPxq);
...
function validarAnulacionOcOpex(btn) {
