...
function editar_opex(idOpex) {
    $.ajax({
        ...
        data.eventoOpex.forEach(element => console.log(option.push(element.idSubproyecto)));
        if (data.cuentaOpex) {
            $("#inputCecoAdd").prop("disabled", true);
            $("#inputCuentaAdd").prop("disabled", true);
            $("#inputAreaFuncional").prop("disabled", true);
            $('#inputCecoAdd').val(data.cuentaOpex[0]['ceco']);
            $('#inputCuentaAdd').val(data.cuentaOpex[0]['cuenta']);
            $('#inputAreaFuncional').val(data.cuentaOpex[0]['areaFuncional']);
            $('#inputMontoFinalAdd').val(data.cuentaOpex[0]['monto_inicial']);
            $('#inputMontoFinalorigin').val(data.cuentaOpex[0]['monto_inicial']);
            $('#inputMontoTemporalorigin').val(data.cuentaOpex[0]['monto_temporal']);
...
function updateCuentaOpex(idOpex) {
    ...
    var inputAreaFuncional = $('#inputAreaFuncional').val();
    var inputMontoFinalAdd = $('#inputMontoFinalAdd').val();
    var inputMontoFinalorigin = $('#inputMontoFinalorigin').val();
    var selectEventoAdd = $('#selectEventoAdd').val();
    var inputDescAdd = $('#inputDescAdd').val();
    var selectAnhoAdd = $('#selectAnhoAdd').val();
    var inputMontoTemporalorigin = $('#inputMontoTemporalorigin').val();

    console.log(parseFloat(inputMontoFinalAdd));
    console.log(parseFloat(inputMontoFinalorigin));
    if (parseFloat(inputMontoFinalAdd) < parseFloat(inputMontoFinalorigin)) {
        swal('Mensaje', 'El monto configurado debe ser mayor a : s./' + inputMontoFinalorigin, 'warning');
        return false;
    }
    ...
    if (inputCuentaAdd === '' || inputCuentaAdd === null) {console.log("VALID2");
        swal('Mensaje', 'Ingrese cuenta ceco', 'warning');
        $("#inputCuentaAdd").css("border", "1px solid red ");
        $("#inputCuentaAdd").css("border-radius", "10px");
        $("#inputCuentaAdd").focus();
    ...
    if (inputAreaFuncional === '' || inputAreaFuncional === null) {console.log("VALID3");
        swal('Mensaje', 'Ingrese area funcional', 'warning');
        $("#inputAreaFuncional").css("border", "1px solid red ");
        $("#inputAreaFuncional").css("border-radius", "10px");
        $("#inputAreaFuncional").focus();
        setTimeout(function () {
    ...
    if (inputMontoFinalAdd === '' || inputMontoFinalAdd === null) {console.log("VALID4");
        swal('Mensaje', 'Ingrese cuenta ceco', 'warning');
        $("#inputMontoFinalAdd").css("border", "1px solid red ");
        $("#inputMontoFinalAdd").css("border-radius", "10px");
        $("#inputMontoFinalAdd").focus();
    ...
    if (selectAnhoAdd === '' || selectAnhoAdd === null) {
        ...
        var monto = parseFloat(inputMontoFinalAdd) - parseFloat(inputMontoFinalorigin);
        var dataString = 'inputCecoAdd=' + inputCecoAdd
                + '&inputMontoTemporalorigin=' + inputMontoTemporalorigin
                + '&montoAdicional=' + monto
                + '&inputMontoFinalorigin=' + inputMontoFinalorigin
                + '&inputCuentaAdd=' + inputCuentaAdd
                + '&inputAreaFuncional=' + inputAreaFuncional
                + '&inputMontoFinalAdd=' + inputMontoFinalAdd
                + '&selectEventoAdd=' + selectEventoAdd
...
function historialOpex(ceco, cuenta, area) {
    $.ajax({
        url: 'historiaTransItemplan',
        type: "POST",
        data: {ceco: ceco, cuenta: cuenta, area: area},
        dataType: "JSON",
...
function historialLogOpex(idOpex) {
    $.ajax({
        url: 'historiaLogOpex',
        type: "POST",
        data: {idOpex: idOpex},
...
//            $('#cabecera').html("HISTORIAL DEL REQUERIMIENTO #" + id);
            if (data.error == 0) {
                $('#modal_form_log').modal('show');
                $('#contTablaLog').html(data.tablaLog);
                initDataTable('#tbOpexDetalleLog');
            } else {
                mostrarNotificacion('error', data.msj, 'error');
            }
        }
