    	...
    	.bootstrapValidator({
    	    	...
    	    	selectSubPro: {
      	    	 ...
      	    	 selectTipoVia: {
      	    	  ...
      	    	  txt_dep_habitados: {
        	            validators: {
        	                notEmpty: {
        	                    message: '<p style="color:red">(*) Debe Ingresar un Número de departamentos habitados.</p>'
        	                }
        	             }
         	    	   }, 
      	    	  txt_avance: {
        	            validators: {
        	                notEmpty: {
        	                    message: '<p style="color:red">(*) Debe Ingresar un avance.</p>'
        	                }
        	             }
         	    	   }, 
      	    	  txt_fec_termino: {
        	            validators: {
        	                notEmpty: {
        	                    message: '<p style="color:red">(*) Debe Ingresar una fecha de término</p>'
        	                }
        	             }
         	    	   }, 
      	    	  inputObservacion: {
        	            validators: {
        	                notEmpty: {
        	                    message: '<p style="color:red">(*) Debe Ingresar una Observación.</p>'
        	                }
        	             }
         	    	   },*/ 	    	   	  
    	    	txt_ruc: {    	          
	           ...
	           /*
	    		...
	    		selectEECC : {
                 ...
                 seletDistrito: {
                     validators: {
                         notEmpty: {
                             message: '<p style="color:red">(*) Debe seleccionar un Distrito.</p>'
                         }
                     }
                 }
    		...
    		e.preventDefault();
			$idAbrePNormal = $('#cmbSubProyecto option:selected').val();

			if($idAbrePNormal == 395 || $idAbrePNormal == 463) {//SI ES DE TIPO ABRE PUERTA 25.07.2019 CZAVALCAS
				swal({
					...
					confirmButtonText: 'Si, Actualizar y Enviar los Datos!',
					cancelButtonClass: 'btn btn-secondary',
					cancelButtonText: 'Si, Solo Actualizar los datos!',
					...
					formData.append('subProyectoDesc', $('#cmbSubProyecto option:selected').text());
					formData.append('selectEECC', $('#selectEECC option:selected').val());
					formData.append('selectDistrito', $('#selectDistrito option:selected').val());
					...
					$.ajax({
					  ...
					  //console.log(dismiss);
					   if(dismiss == 'cancel'){//solo guardar datos
						...
						formData.append('tipo_save', 2);//solo guardar datos
						$.ajax({
							data: formData,
							url: "updateCV2",
							cache: false,
			...
			} else {//SI ES DE TIPO PORCENTAJE
			var avance_select = $('#txt_avance').val();
				if(avance_select >= 50){//si es mayor igual a 50 actua como abre puertas
					swal({
						title: 'Est&aacute; seguro de ejecutar los cambios?',
						...
						formData.append('plan_id_mes'   , $('#selectPlan option:selected').val());
				...
				}else{//si es menor a 50 , nullo o vacio funciona como porcentaje normal
				var $form    = $(e.target),
    	        formData = new FormData(),
    	        params   = $form.serializeArray(),
    	        bv       = $form.data('bootstrapValidator');
