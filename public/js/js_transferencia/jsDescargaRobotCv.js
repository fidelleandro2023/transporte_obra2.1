...
function init(){
	...
	var triangleCoords = [
		new google.maps.LatLng(-12.043053103865134, -77.03242039038777),
		new google.maps.LatLng(-12.048917478865134, -77.02333993940591),
		new google.maps.LatLng(-12.045212625263796, -77.0333568166101)
	];
...
function getPostesMap() {
	...
	$.ajax({
		...
		$('#barra_prog').css('width', '70%');
		var marcaPoste = [];
		data.objDataPoste.forEach(function(dataArray, index){
			var temp = {
							'codigo'   : dataArray.codigo,
							'longitud' : dataArray.longitud,
							'latitud'  : dataArray.latitud,
							'icon_poste' : data.img
						}
						
			marcaPoste.push(temp);			
			// $temp = array(
                            // 'codigo'       => $row['codigo'],
		...
		for( i = 0; i < marcaPoste.length; i++ ) {
			var position = new google.maps.LatLng(marcaPoste[i].latitud, marcaPoste[i].longitud);

			marcadores = new google.maps.Marker({
													icon: marcaPoste[i].icon_poste,/* icon_url_cto_gbl, */
													position: position,
													map: map,
													title: marcaPoste[i].codigo
												});
			 // Add info window to marker    
			google.maps.event.addListener(marcadores, 'click', (function(marcadores, i) {
				
				return function() {
					//infoWindowMarcadores.setContent(data.infoMarcaPoste[i][0]);
					infoWindowMarcadores.open(map, marcadores);
				}
			})(marcadores, i));
