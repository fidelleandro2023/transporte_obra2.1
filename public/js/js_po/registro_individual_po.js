...
function guardarPO() {
    if (arrayMatGlob.length > 0 && fromGlob != null && costoTotalMat != null) {
		...
		//nuevo canCreateEditPOByCostoUnitario czavalacas30.01.2020 hereda de Utils.js
        console.log('itemplanGlob:'+itemplanGlob);
		console.log(arrayMatGlob);
		
		if(itemplanGlob == null || itemplanGlob == '') {
			return;
		}
		
		if(idEstacionGlob == null || idEstacionGlob == '') {
			return;
		}
		...
		jsonCreateSol = { origen       		: 1,
						  ...
						  costoTotalPo_dato : costoTotalMat, 
						  data_json         : arrayMatGlob,
						  idEstacion        : idEstacionGlob };
									  
        canCreateEditPOByCostoUnitario(jsonCreateSol, function() {//reactivado el 18.03.2020 solo para sisegos czavalacas
			$.ajax({
				type: 'POST',
				url: 'registPO',
				data: {
