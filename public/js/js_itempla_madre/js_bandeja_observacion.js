...
function  modalItemplanHijos(itemplanMadre) {
    ...
    $.ajax({
        ...
        if (data.error == 0) {
            ...
            $("#titel_hijos").html('ITEMPLAN HIJOS DEL ITEMPLAN MADRE : ' + itemplanMadre);
...
function  modalValiOpe(itemPlan, number) {
    ...
    if (number == '1') {
        $("#title_vali").html('OBSERVACION DE DISE\u00D1O');
        $("#modalVali").modal('show');
        $('#boton_multiuso').attr("onclick", 'validarOpex("' + itemPlan + '","estado_di")');

    } else {

        $("#title_vali").html('OBSERVACION DE OPERACION');
        $("#modalVali").modal('show');
        $('#boton_multiuso').attr("onclick", 'validarOpex("' + itemPlan + '","estado_ope")');
    }
}

function validarOpex(itemPlan, colum) {
    console.log(itemPlan);
    var textComentario = $('#textComentario').val();
    var selectEstado = $('#selectEstado').val();

    if (textComentario === '' || textComentario === null) {
        swal('Mensaje', 'Ingrese comentario', 'warning');
        $("#inputCecoAdd").focus();
        return false;
    }
    if (selectEstado === '' || selectEstado === 0) {
        swal('Mensaje', 'Selecione accion', 'warning');
        $("#selectEstado").focus();
        return false;
    } else {
        $.ajax({
            url: 'itObs',
            type: "POST",
            data: {itemPlan: itemPlan, textComentario: textComentario, colum: colum},
            dataType: "JSON",
            success: function (data)
            {
                if (data.error == 0) {
                    ...
                    swal('Mensaje', 'Se valido correctamente', 'success');
                    location.reload();
