...
function validarEdicionOc(component) {
  ...
  swal({
    ...
    html:
      ...
      '<label>Costo SAP :</label><input type="text" class="form-control col-md-4" placeholder="Ingresar costo sap.." style="background:#F9F8CF" id="costoSap" />' +
      '</div>',
    ...
    $.ajax({
      type: 'POST',
      url: 'valSolEdiOC',
      data: { txtOC: hg, costoSap: costoSap },
      async: false,
    })
      .done(function (data) {
...
function getPtrByHojaGestion(component) {
  ...
  $.ajax({
    type: 'POST',
    url: 'getPtrByHGestionOC',
    data: { idhg: idhg, hg: hg, ts: ts },
  }).done(function (data) {
    data = JSON.parse(data);
    ...
    if (data.error == 0) {
      ...
      $('#conTablaDetallePO').html(data.tablaDetallePo);
      console.log(data.tablaDetallePo);
      initDataTable('#tb_detalle_po');
      /*
...
function filtrarTablaHG() {
  ...
  $.ajax({
    type: 'POST',
    url: 'filtrarTablaCOC',
    data: { soli: soli, item: item, estado: estado },
  }).done(function (data) {
    data = JSON.parse(data);
    if (data.error == 0) {
...
$('#sendEnProcesoForm')
  ...
  .on('success.form.bv', function (e) {
    ...
    swal({
      title:
        'Est&aacute; seguro de cambiar de estado a "EN PROCESO" la Hoja de Gestion ' +
        hojaGestion +
        '?',
...
var itemplanGlobalSol = null;
function openModalQuiebre(btn) {
  let codigoSolicitud = btn.data('codigoSolicitud');
  itemplanGlobalSol = btn.data('itemplan');

  if (itemplanGlobalSol == null || itemplanGlobalSol == '') {
    console.log('Angeaaaa');
    return;
  }

  $.ajax({
    type: 'POST',
    url: 'openModalQuiebreSolOc',
  }).done(function (data) {
    data = JSON.parse(data);
    $('#contCmbMotivo').html(data.cmbMotivo);
    $('#txtComentario').val('');
    $('#txtCodigoSolicitud').val(codigoSolicitud);
    modal('modaldQuiebre');
  });
}
...
function enviarQuiebreSolOc() {
  swal({
    title: 'Est&aacute; seguro de mandar a la bandeja de quiebre?',
    text: 'Asegurese de que la informacion sea la correta.',
    type: 'warning',
    showCancelButton: true,
    buttonsStyling: false,
    ...
    function () {
      let txtCodigoSolicitud = $('#txtCodigoSolicitud').val();
      let idMotivo = $('#contCmbMotivo option:selected').val();
      let desMotivo = $('#contCmbMotivo option:selected').text();
      ...
      if (txtComentario == null || txtComentario == '') {
      ...
      $.ajax({
        type: 'POST',
        url: 'enviarQuiebreSolOc',
        data: {
          codigoSolicitud: txtCodigoSolicitud,
          itemplan: itemplanGlobalSol,
          idMotivo: idMotivo,
          desMotivo: desMotivo,
        ...
        if (data.error == 0) {
          swal({
            title: 'Se atendio Correctamente',
            text: 'Aparecera la solicitud en la bandeja de quiebre!',
            type: 'success',
            buttonsStyling: false,
            confirmButtonClass: 'btn btn-primary',
            confirmButtonText: 'OK!',
