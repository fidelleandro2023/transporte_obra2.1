    ...
    methods:{
        ...
        registrarTrama:function() {
            ...
            $.ajax({
                ...
                data = JSON.parse(data);
                if(data.error == 0) {
                    mostrarNotificacion('error', 'Se registr&oacute; correctamente');
                    location.reload();
                } else {
...
if(itemPlanFormPorcentaje == null ||itemPlanFormPorcentaje == '') {console.log("ENTRo1");
    return;
} 

if(descEstacion == null || descEstacion == '') {console.log("ENTRo2");
    return;
}

if(idEstacion == null || idEstacion == '') {console.log("ENTRo3");
    return;
}

if(idZonal == null || idZonal == '') {console.log("ENTRo4");
    return;
}

if(idProyectoGlobal == null || idProyectoGlobal == '') {console.log("ENTRo5");
    return;
}
            ...
            if(flgDocGlobal == null || flgDocGlobal == '' || flgDocGlobal == undefined) {
                // if (navigator.geolocation) {
                    // navigator.geolocation.getCurrentPosition(function(position) {
                        ...
                        /*
						if(x==null || x==''|| y==null || y=='') {
                            mostrarNotificacion('error', 'No se ingreso las coordenadas');
                            return;
                        }*/
                ...
                // } 
                // else {
                //     console.log("no soportada por el navegador");
                // }
            }
...
//         var submitButton = document.querySelector("#btnAceptarSubirFoto")
//         var submitBtnAct = document.querySelector("#btnAceptarSubirAct")
//         myDropzone = this; 
...
//         );
//         submitBtnAct.addEventListener("click", function(){
//             myDropzone.processQueue(); 
//         });  
...
function openModalBandejaEjecucionFuera(btn) {
    ...
    $.ajax({
        ...
        if(newData != null) {
            Object.keys(newData).forEach(function(key){
                button+='<button class="btn btn-success" data-nombre="'+newData[key]+'" data-key_json="'+key+'" onclick="openModalDeleteArchFoto2($(this))">'+newData[key]+'</button>';
            });
        }
...
/************nuevo liquidacion OC 08.08.2019 czavala ******************/
var listJsonPartidasOC = null;
function liquidarOC(btn) {
	var itemPlan = btn.data('itemplan');
	var idSubPro = btn.data('idsubpro');
	var accion	 = btn.data('accion');
	listJsonPartidasOC	= null;
	 $.ajax({
	            type : 'POST',
	            url  : 'makeHtmlLiq',
	            data : {itemplan : itemPlan,
	        ...
	        data = JSON.parse(data);
	        if(data.error == 0){	        	
	        	$('#bodyTableLiq').html(data.htmlConTabla);	            
	        	soloDigitos('canclass');
	        	listJsonPartidasOC = JSON.parse(data.listItemPartidas);
	        	for ( var i = 0; i < listJsonPartidasOC.length; i++ ) {
	        	    //console.log(listJsonParti[i]);	        	
		        	$('#forRegistrarLiqOC').bootstrapValidator('addField', 'cantTotal'+listJsonPartidasOC[i], {
	                    validators: {
	                    /*	integer : {
	                            message : '<p style="color:red">(*)Por favor ingresa un valor numerico entero permitido</p>'
	                        },*/
	                		callback: {
	    	                    message: '<p style="color:red">(*)Ingresar una Cantidad Mayor o igual a 0</p>',
	    	                    callback: function(value2, validator) { 
	        	                    console.log('value:'+value2);  	        	                    
        	                    	if(value2 >= 0){//console.log('value:'+value["max"]);
        	                    		console.log('value true:'+value2);  
        	                    		return true;	        	                    		
        	                    	}else{
        	                    		console.log('value false:'+value2);  
        	                    		return false;
        	                    	}	        	                   	
	    	                    }
	    	                }    	                
	        	...
	        	$('#btnLiqOC').attr('data-itemplan',data.itemplan);     
	            $('#btnLiqOC').attr('data-accion',data.accion);     
	            $('#btnLiqOC').attr('data-idSubPro',data.idsubPro);    
	        	$('#modalLiqOC').modal('toggle');
	        }
	    });
}
...
var $form    = $(e.target),
    ...
    var idSubPro	=	$('#btnLiqOC').attr('data-idSubPro');
    formData.append('idSubPro', idSubPro);
    
    var itemplan	=	$('#btnLiqOC').attr('data-itemplan');
    formData.append('itemplan', itemplan);
    
    var accion		=	$('#btnLiqOC').attr('data-accion');
    formData.append('accion', accion);
    
    swal({
        title: 'Está seguro de liquidar la OC?',
        text: 'Asegurese de que la informacion llenada sea la correta.',
        type: 'warning',
        showCancelButton: true,
        buttonsStyling: false,
...
function calcularTotalPartida(id_item_partida){
	var monto = $('#montTh'+id_item_partida).text();
	var cantidad = $('#cantTotal'+id_item_partida).val();	
	var montoTotal = (Number(cantidad) * Number(monto)).toFixed(2);
	$('#contTh'+id_item_partida).html(montoTotal);
	
	var montoTotalFinal = 0;
	if(listJsonPartidasOC != null){
		for ( var i = 0; i < listJsonPartidasOC.length; i++ ) {
			montoTotalFinal = montoTotalFinal + Number($('#contTh'+listJsonPartidasOC[i]).text());
		}
		
		$('#costoTotalPar').html(montoTotalFinal.toFixed(2));
	}
