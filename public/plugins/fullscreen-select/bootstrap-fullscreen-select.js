...
//function activarBtnAgregarInstrumento(){
//	var cantPalabra = $('#nameInstrumento').val();
//
//	if( (cantPalabra.length) > 0 ){
//		$('#nuevoInstrumento').css("background-color", "#FF9200 !important");
//		$('#nuevoInstrumento').find("i").css("color", "#FFFFFF");
//		$('#nuevoInstrumento').removeAttr('disabled');
//		$('#nuevoInstrumento').addClass('mdl-button--raised');
//	}else if ( (cantPalabra.length) == 0){
//		$('#nuevoInstrumento').css("background-color", "#EDEDED");
//		$('#nuevoInstrumento').find("i").css("color", "#757575");
//		$('#nuevoInstrumento').attr('disabled');
//		$('#nuevoInstrumento').removeClass('mdl-button--raised');
//	}
//}
    ...
    $.fn.mobileSelect = function (options) {
        ...
        if (typeof options === 'string') {
            ...
            if (options === 'refresh2') {
                ...
                $this.each(function (i, a) {
                    var id = $(a).attr('data-msid');
                    $.mobileSelect.elements[id].refresh2();
                });
            }
            if (options === 'hide') {
        ...
        //CREACION DE TEMPLATE DINAMICO
        var display = (options.instrumento == true) ? 'none' : 'block';
        var content = '<div class="list-container"></div>';
        if(options.instrumento == true){
        	content = '<div class="mdl-tabs mdl-js-tabs mdl-js-ripple-effect">'+
				          '<div class="mdl-tabs__tab-bar">'+
				              '<a id = "tabSelectInstr" href="#selecInstrumento" class="mdl-tabs__tab is-active">Selec. Instrumento</a>'+
				              '<a id = "tabCrearInstr" href="#crearInstrumento" class="mdl-tabs__tab">Crear Instrumento</a>'+
				          '</div>'+
				          '<div class="mdl-tabs__panel is-active" id="selecInstrumento">'+
				              '<div class="row p-0 m-0">'+
					              '<div class="list-container"></div>'+
				              '</div>'+
				          '</div>'+
				          '<div class="mdl-tabs__panel" id="crearInstrumento">'+
				              '<div class="row-fluid p-r-20 p-l-20">'+
				                  '<div class="col-sm-12 p-0 mdl-input-group mdl-input-group__text-btn">'+				               
				                      '<div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label">'+
				                          '<input class="mdl-textfield__input" type="text" maxlength="200" name="nombreInstrumento" id="nombreInstrumento" onkeyup="activarBtnAgregarInstrumento()" value="">'+
				                          '<label class="mdl-textfield__label" for="nombreInstrumento">Crea un Instrumento</label>'+
				                      '</div>'+
				                      '<div class="mdl-btn">'+
						                  '<button class="mdl-button mdl-js-button mdl-button--icon mdl-button--raised" id="nInstrumento" name="nInstrumento" onclick="" disabled>'+
							                  '<i class="mdi mdi-add" id="icon_save_instru"></i>'+
						                  '</button>'+
						              '</div>'+
				                  '</div>'+
				              '</div>'+
				              '<div class="row-fluid p-r-20 p-l-20">'+
		                          '<div class="col-sm-12" id="contTableInstru"></div>'+
		                      '</div>'+
				          '</div>';
        }
        var template = '<div>'+content+' <div class="mobileSelect-title" style="display:'+display+'"></div><div class="mobileSelect-buttons"><a href="#" class="mobileSelect-cancelbtn"></a><a href="#" class="mobileSelect-savebtn"></a><a href="#" class="mobileSelect-clearbtn"></a></div></div>'
        //SET TEMPLATE DINAMICO
        $.fn.mobileSelect.defaults.template = template;
        //FIN CONFIGURACION
    ...
    Mobileselect.prototype = {
        ...
        _buildHTML: function () {
            ...
            if (this.$e.attr('data-triggerOn') === undefined) {
                ...
                this.$e.before('<button type="button" class="btn ' + this.style + ' btn-mobileSelect-gen form-control" '+b+' ><span class="text"></span> <span class="caret"></span></button>');
        ...
        _appendOptionsList: function () {
            ...
            $.each(this.options, function (i, a) {
                ...
                that.$listcontainer.append('<a href="#" class="mobileSelect-control" '+ b +' data-value="' + a.value + '" data-texto="'+a.text+'">' + a.text + '</a>');
...
//                    var label = $t.parent().is('optgroup') ? $t.parent().attr('label') : false;
     ...
     * plugin defaults
     */
    var crearInstru = (cont == 5) ? '<div class="mdl-tabs mdl-js-tabs mdl-js-ripple-effect">'+
			              '<div class="mdl-tabs__tab-bar">'+
			                  '<a href="#selecInstrumento" class="mdl-tabs__tab">Selec. Instrumento</a>'+
			                  '<a href="#crearInstrumento" class="mdl-tabs__tab">Crear Instrumento</a>'+
			              '</div>'+
			              '<div class="mdl-tabs__panel is-active" id="selecInstrumento">'+
			                  '<div class="row p-0 m-0">'+
			        	          '<div class="list-container"></div>'+
			              ...
			              '<div class="mdl-tabs__panel" id="crearInstrumento">'+
			                  '<div class="row-fluid p-r-20 p-l-20">'+
				                  '<div class="col-sm-12 p-0 mdl-input-group mdl-input-group__text-btn">'+				               
					                  ...
					                  '</div>'+
					              '<div class="mdl-btn">'+
							          '<button class="mdl-button mdl-js-button mdl-button--icon mdl-button--raised" id="nInstrumento" name="nInstrumento">'+
								          '<i class="mdi mdi-add"></i>'+
							          '</button>'+
							      '</div>'+
					          '</div>'+
			              '</div>'+
                      '</div>' : '<div class="list-container"></div>';
