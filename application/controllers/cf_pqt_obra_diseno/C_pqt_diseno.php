...
class C_pqt_diseno extends CI_Controller {
    ...
    function getTablaDiseno($itemplan) {
        $dataDiseno = $this->m_pqt_diseno->getDataDiseno($itemplan);
	$btnClose = '';
        ...
        foreach ($dataDiseno as $row) {
            $btnFormulario = '';
	   if ($row->estado == ESTADO_PLAN_DISENO && $this->m_pqt_diseno->existenAunEnPreDiseno($row->itemPlan) == 1) {
                $btnClose = '<a onclick="updatePartidas(' . "'" . $row->itemPlan . "'" . ',' . "'" . $row->idEstacion . "'" . ',' . "'" . $row->idProyecto . "'" . ')"><img alt="Cancelar" height="20px" width="20px" src="public/img/iconos/cancelar.png" title="Esta estacion no se requiere"></a> ';
            }
            ...
            if ($row->estado == ESTADO_PLAN_DISENO) {
                //comentado por Ivan More Flores
//                if ($row->flg_orden_compra == 0 && $row->idProyecto == 4) {
//                    $itemplan = $row->itemPlan;
//                } else {
//                    $itemplan = '<a href="#"  class="ver_ptr" data-idrow="' . $row->itemPlan . $row->idEstacion . '" data-estacion="' . $row->idEstacion . '">' . $row->itemPlan . '</a>';
//                }
                if ($row->flg_orden_compra !== 0 || $row->idProyecto == 4) {
                    $itemplan = '<a href="#"  class="ver_ptr" data-idrow="' . $row->itemPlan . $row->idEstacion . '" data-estacion="' . $row->idEstacion . '">' . $row->itemPlan . '</a>';
                } else {
            ...
            } else { // OBRAS PUBLICAS Y SIN ORDEN DE COMPRA
                $itemplan = $row->itemPlan;
            }

            if ($row->estado == ESTADO_PLAN_DISENO && $row->flg_orden_compra == 1 && $row->idEstadoPlan != ID_ESTADO_CANCELADO && $row->idEstadoPlan != ID_ESTADO_TERMINADO && $row->idEstadoPlan != ID_ESTADO_CERRADO && $row->idEstadoPlan != ID_ESTADO_TRUNCO && $row->idEstadoPlan != ID_ESTADO_SUSPENDIDO) {
                $btnCheck = '<a data-item="' . $row->itemPlan . '" data-sisegofg="' . ($row->idProyecto == ID_PROYECTO_SISEGOS ? 1 : 0) . '" data-id_estacion="' . $row->idEstacion . '" onclick="abrirModalAsignarEntidades(this)" style="margin-left: 10%;"><img alt="Editar" height="20px" width="20px" src="public/img/iconos/check_24016.png"></a> ';
            } else if ($row->idProyecto == ID_PROYECTO_OBRA_PUBLICA && $row->flg_orden_compra == 0) {
                $btnCheck = '<a data-item="' . $row->itemPlan . '" data-sisegofg="' . ($row->idProyecto == ID_PROYECTO_SISEGOS ? 1 : 0) . '" data-id_estacion="' . $row->idEstacion . '" onclick="abrirModalAsignarEntidades(this)" style="margin-left: 10%;"><img alt="Editar" height="20px" width="20px" src="public/img/iconos/check_24016.png"></a> ';
            } else {
                $btnCheck = null;
            }
...
//                $btnCheck = null;
//            }
            //<td>' . $btnCheck . (($row->estado == 3) ? 'EXPEDIENTE <a href="'.$row->path_expediente_diseno.'" download=""><i class="zmdi zmdi-hc-2x zmdi-download"></i></a>'.$btnInfo : '').'</td>
            ...
            $html .= '<tr id="' . $row->itemPlan . $row->idEstacion . '"> 
                            <td>' . $btnClose . $btnCheck . (($row->estado == 3) ? 'EXPEDIENTE <a href="' . $row->path_expediente_diseno . '" download=""><i class="zmdi zmdi-hc-2x zmdi-download"></i></a>' . $btnInfo : '') . '</td>
                            <th>' . $itemplan . '</th>	
    ...
    public function ejecutarPqtDiseno() {
        ...
        try {
            ...
            if ($idProyecto == ID_PROYECTO_OBRA_PUBLICA) {
                $count = $this->m_pqt_diseno->countLicencia($itemplan, $numEntidadesSelec);
                if ($count === false) {
                    throw new Exception('El SubProyecto No cuenta con Partidas configuradas para la creacion de Licencia.');
                }
            }
...
//                    $this->cambiarDisenoObra($itemplan, $estado_plan); //registra log en planobra por el estado en licencia y en log planobra bandeja ejecucion
                    ...
                    if ($idProyecto == ID_PROYECTO_OBRA_PUBLICA) {
                        if ($this->m_pqt_diseno->existenAunEnPreDiseno($itemplan) == 0) {
                            $this->cambiarDisenoObra($itemplan, $estado_plan, ID_ESTADO_DISENIO_EJECUTADO);
                        }
                        $this->m_pqt_diseno->simularLicencia($itemplan, $numEntidadesSelec);
                        $this->m_pqt_diseno->simularLicenciaDetalle($itemplan, $numEntidadesSelec);
                    } else {
                        $this->cambiarDisenoObra($itemplan, $estado_plan, ID_ESTADO_EN_LICENCIA);
                    }
                }
                #}
            }
            ...
            $operador = $this->m_utils->getOperador($itemplan);

            if ($operador == 'Estudio de Campo') {
                $arrayUpd = array('idEstadoPlan' => 4,
                    'usu_upd' => $this->session->userdata('idPersonaSession'),
                    'fecha_upd' => $this->fechaActual(),
                    'descripcion' => 'TERMINADO - ESTUDIO DE CAMPO');
                $data = $this->M_bandeja_itemplan_estacion->editarPlanObra($itemplan, $arrayUpd);
            }
    ...
    function updateObra() {
        ...
        try {
            $itemplan = $this->input->post('itemplan');
            $idEstacion = $this->input->post('estacion');
            $idProyecto = $this->input->post('idProyecto');

            if ($idProyecto == ID_PROYECTO_OBRA_PUBLICA) {
                $this->cambiarDisenoObra($itemplan, 2, ID_ESTADO_DISENIO_EJECUTADO);
            } else {
                $this->cambiarDisenoObra($itemplan, 2, ID_ESTADO_EN_LICENCIA);
            }

            $this->m_pqt_diseno->updateEstadoPlanObraToBanEjecucion($itemplan, 3, $idEstacion);
            $data['error'] = EXIT_SUCCESS;
        } catch (Exception $e) {
            $data['msj'] = $e->getMessage();
        }
    ...
    function getCountPartida() {
        ...
        try {
            $itemPlan = $this->input->post('itemplan');
            $data['error'] = EXIT_SUCCESS;
            $data['info'] = $this->m_pqt_diseno->existenAunEnPreDiseno($itemPlan);
        } catch (Exception $e) {
            $data['msj'] = $e->getMessage();
        }
        echo json_encode($data);
    ...
    function cambiarDisenoObra($itemPlan, $estadoActual, $idEstadoPlan) {
        $this->m_pqt_diseno->actualizarEstadoPlanObraADiseno($itemPlan, $idEstadoPlan); //log en licencia
        $arrayDataLog = array(
            'tabla' => 'planobra',
