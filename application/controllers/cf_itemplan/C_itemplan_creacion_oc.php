            ...
            foreach ($listaHojaGestion as $row) {
                ...
                if ($row->estado == 1) {//PENDIENTE
                    if ($row->tipo_solicitud == 1) {
                        ...
                        $btnEnProceso = '<a data-hg="' . $row->id . '" data-codigo_sol="' . $row->codigo_solicitud . '" onclick="validarAnulacionOcOpex($(this))"><i title="Validar OC" style="color:#A4A4A4" class="zmdi zmdi-hc-2x zmdi-case-check"></i></a>';
                    }
...
function opex_desc($id) {
    ...
    function validarSolicitudEdicionOC() {
        ...
        try {
            ...
            if ($idPersona != null) {
				...
				$countCerti = $this->m_utils->getCountTipoSolicitud($itemplan, 3, 4);
    
				if($countCerti > 0) {
					$arrayDataIte['estado_oc_certi'] = 'PENDIENTE';
					 $arrayDataCerti = array(
												'estado' => 1
											);
											
					$estadoActual = 4;
					$estadoFinal  = 1;
					$data = $this->m_utils->updateSolicitudOcByItemplan($itemplan, 3, $estadoActual, $estadoFinal);
                ...
                $data = $this->m_itemplan_creacion_oc->update_itemplan($arrayDataItem, $itemplan);
	...
	function validarAnulacionOcOpex() {
		$data['msj'] = null;
        $data['error'] = EXIT_ERROR;
        try {
            ...
            if($idPersona != null){
                    ...
                    $dataPlanObra = array('solicitud_oc_anula_pos'	=> $codigo_solicitud,
                    ...
                    $data = $this->m_itemplan_creacion_oc->update_solicitud_oc_anulado($codigo_solicitud, $arrayData, $dataPlanObra);   

                    $obrasAfectadas = $this->m_itemplan_creacion_oc->getObrasToEvaluarTrunco($codigo_solicitud);
                    foreach($obrasAfectadas as $obra){
                        if($obra->idEstadoPlan ==   ID_ESTADO_TRUNCO){//POR AHORA SOLO FO EJECUTA PAGO TRUNCO HFC EN VEREMOS.
                            $this->m_pqt_trunco->evaluarObraTrunca($obra->itemplan, ID_ESTACION_FO);                            
                        }
                    }
                    /***********************************/
