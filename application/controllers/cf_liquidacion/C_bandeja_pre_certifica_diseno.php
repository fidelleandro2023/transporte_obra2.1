 ...
 *
 */
class C_bandeja_pre_certifica_diseno extends CI_Controller {
	...
	{  	   
	    ...
	    if($logedUser != null){
        	   ...
        	   $result = $this->lib_utils->getHTMLPermisos($permisos, ID_PERMISO_PADRE_DISENO, ID_PERMISO_HIJO_BANDEJA_PRE_CERTI_DISENO);
        	   $data['opciones'] = $result['html'];
        	   if($result['hasPermiso'] == true){
        	       $this->load->view('vf_liquidacion/v_bandeja_pre_certifica_diseno',$data);
        	   }else{
    ...
    public function makeHTLMTablaBandejaAprobMo($listaPTR){     
        $html = '<table id="data-table" class="table table-bordered">
                ...
                $html .=' <tr>
                            <th>'.(($row->hasDise=='0') ? '<img alt="Editar" height="20px" width="20px" src="public/img/iconos/circle-check-128.png">' : '').'</th>
                            <th>'.(($row->hasExpe=='1') ? '<img alt="Editar" height="20px" width="20px" src="public/img/iconos/expediente.png">' : '').'                                </th>
