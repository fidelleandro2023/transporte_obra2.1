...
class C_pqt_liquidacion_mat extends CI_Controller {

	function __construct(){
        parent::__construct();
        $this->output->set_header('Content-Type: text/html; charset=ISO-8859-1');
        $this->load->model('mf_pqt_liquidacion_mat/m_pqt_liquidacion_mat');
    ...
    public function index(){
	    ...
	    if($logedUser != null){
        	   ...
        	   if($result['hasPermiso'] == true){
        	         $this->load->view('vf_pqt_liquidacion_mat/v_pqt_liquidacion_mat',$data);
        	   }else{
    ...
    function filtrarTablaSimuladorMO() {
        ...
        try {
            ...
            $data['tablaBandejaHG'] = $this->getTablaHojaGestion($this->m_pqt_liquidacion_mat->getBandejaSimulador($criterios, $promotor));
    ...
    function getPtrsByHojaGestion() {
        ...
        try {
            ...
            $listaEstado = $this->m_pqt_liquidacion_mat->getPtrsByHojaGestion($hojaGestion);
            $infoHg = $this->m_pqt_liquidacion_mat->getBolsaHgDataByHG($id_hoja_gestion);
    ...
    function filtrarTablaHG() {
        ...
        try {
            ...
            $data['tablaBandejaHG'] = $this->getTablaHojaGestion($this->m_pqt_liquidacion_mat->getBandejaHojaGestion($tipoObra, $eecc, $estado, $hojaGestion, $codigoPo, $cesta));
            $data['error'] = EXIT_SUCCESS;
        } catch(Exception $e) {
            $data['msj'] = $e->getMessage();
        }
    ...
    function setHojaGestionEnProceso() {
        ...
        try {
            ...
            $infoHg = $this->m_pqt_liquidacion_mat->getBolsaHgDataByHG($idHojaGestion);
            if($infoHg['estado']==1){
                ...
                $data = $this->m_pqt_liquidacion_mat->updateHojaGestion($idHojaGestion, $arrayData);
            }else{
	...
	public function makeCSVHojaGestionMO(){
        ...
        try{
            ...
            $detalleplan = $this->m_pqt_liquidacion_mat->getDataToExcelReport();
