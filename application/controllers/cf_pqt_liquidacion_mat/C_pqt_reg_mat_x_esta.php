...
class C_pqt_reg_mat_x_esta extends CI_Controller {
    ...
    public function index(){
        ...
        if($logedUser != null){
           ...
           // if($result['hasPermiso'] == true){//SI CUENTA CON LOS PERMISOS RECIEN INGRESAR                
                //PONER VALIDACIONES
                $infoItemEsta = $this->m_pqt_reg_mat_x_esta->getInfoEstacionItemplanToRegMatXEsta($item, $idEstacion);
                ...
                if($infoItemEsta!=null){
					...
					log_message('error','<<<---->');
                    $this->load->view('vf_pqt_liquidacion_mat/v_pqt_reg_mat_x_esta',$data);
                }else{
    ...
    public function getExcelMateriales()
    {
        ...
        try {
            ...
            $arrayMateriales = $this->m_pqt_reg_mat_x_esta->getMaterialesNuevoModelo();			
    ...
    public function makeHTMLBodyTable($row_dimension, $objPHPExcel){
        ...
        for ($i = 1; $i <= $row_dimension['row']; $i++){//COMIENZA DESDE LA FILA 1
            ...
            if($D!='' && $A!='CODIGO MATERIAL'){
                if(is_numeric($D)){
                    $infoPartida    =   $this->m_pqt_reg_mat_x_esta->getInfoCodigoMaterial($A);
                    if($infoPartida!=null){
            ...
            if($idUsuario   !=     null){
                ...
                if (move_uploaded_file($_FILES['fileEvi']['tmp_name'], $uploadfile1)) {
                    ...
                    if($arrayFile!=null){
                        ...
                        $data = $this->m_pqt_reg_mat_x_esta->registrarMaterialesXEstacion($materialesPadre, $arrayFinalInsert, $itemplan, $idEstacion);
    ...
    function getMaterialesPqt() {
        ...
        try {
            if($this->session->userdata('idPersonaSession') != null){
                ...
                $data['tablaMateriales'] = $this->getTablaMateriales($this->m_pqt_reg_mat_x_esta->getMaterialesByIdEstacionItemplan($itemplan, $idEstacion));
                $data['error'] = EXIT_SUCCESS;
            }else{
