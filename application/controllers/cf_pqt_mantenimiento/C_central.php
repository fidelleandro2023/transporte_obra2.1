...
class C_central extends CI_Controller {
	...
	{  	   
	    ...
	    if($logedUser != null){           
               $data['listartabla'] = $this->makeHTMLTablaCentral($this->m_pqt_central->getAllCentrales());
        	   ...
        	   $result = $this->lib_utils->getHTMLPermisos($permisos, ID_PERMISO_PADRE_MANTENIMIENTO_NUEVO_MODELO, ID_PERMISO_HIJO_PQT_MAN_CENTRAL, ID_MODULO_MANTENIMIENTO);
        	   $data['opciones'] = $result['html'];
        	   if($result['hasPermiso'] == true){
        	       $this->load->view('vf_pqt_mantenimiento/v_central',$data);
        	   }else{
    ...
    function existeCodigoCentral(){
        ...
        if($codigoCentral != null){
            $res  = $this->m_pqt_central->existeCentral($codigoCentral);
    ...
    public function createCentral(){
        ...
        try{
            ...
            $data = $this->m_pqt_central->insertarCentral($idTipocentral, $descripcion, $codigo, $zonal, $eecc, $arrayJefatura['descripcion'], $region,$jefatura);
            if($data['error'] == EXIT_ERROR){
                ...
                $data = $this->m_utils->insertarLogPqtCentral($arrayInsertLogCentral);
    ...
    public function getInfoCentral(){
        ...
        try{
            ...
            $central = $this->m_pqt_central->getCentraInfo($id);   
...
//             log_message('error', 'data:'.print_r($data,true).'$id:'.$id);
    ...
    public function editarCentral(){
        ...
        try{
            ...
            $data = $this->m_pqt_central->editarCentralModelo($id, $idTipocentral, $descripcion, $codigo, $zonal, $eecc, $idJefatura, $region, $idJefatura, $descJefatura);
            if($data['error']==EXIT_ERROR){
                throw new Exception('Error al Insertar createCentral');
            }else{
                $arrayInsertLogCentral = array(
