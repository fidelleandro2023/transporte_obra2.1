        ...
        try{
            ...
            if($reject) {
                ...
                if($info['idProyecto'] == ID_PROYECTO_SISEGOS){
                    ...
                    // if($hasFormSisego   ==  0){
                    //     throw new Exception('NO SE PUEDE APROBAR, AGREGUE INFROMACION EN EL FORMULARIO DISEÑO DE OBRA');
                    // }
                }
            }
            $data['error'] = EXIT_SUCCESS;
    ...
    public function ejecutarDiseno(){
        ...
        try{
            ...
            if($data['error'] == EXIT_ERROR){
                throw new Exception('ERROR AL EJECUTAR EL DISEÃ‘O');
            }
        ...
        try {
            ...
            if (is_array($arrayIdEntidades) && $numEntidades > 0) { 
               if($numEntidades ==  1){
                   ...
                   foreach ($arrayIdEntidades as $row) {
                       if($row[0] != 0){//si cuenta con mas de una entidad seleccionada no tomar en cuenta los 0
                                array_push($arrayInsert,array('idEntidad' => $row[0],
                                       'idEstacion' => $idEstacion,
                                       'itemPlan' => $itemplan,
                                       'ruta_pdf' => null,
                ...
                if($infoPlanObra['operador'] == 'Estudio de Campo') {
                    ...
                    if($estado_plan == ESTADO_PLAN_DISENO){
                        ...
                        if($data['error'] == EXIT_SUCCESS) {
                             $arrayDataLog = array(
                                                    'tabla'            => 'planobra',
                                                    'actividad'        => 'Diseno - Bandeja EjecuciÃ³n',
                                                    'itemplan'         => $itemplan,
                                                    'fecha_registro'   => $this->fechaActual(),
                                                    'id_usuario'       => $this->session->userdata('idPersonaSession'),
                        ...
                        if($hasPTRApro!=null){
                            if($hasPTRApro['numAprob']>=1){
                                ...
                                if($data['error'] == EXIT_SUCCESS) {
                                     $arrayDataLog = array(
                                                            'tabla'            => 'planobra',
                                                            'actividad'        => 'Obra - Bandeja EjecuciÃ³n',
                                                            'itemplan'         => $itemplan,
                                                            'fecha_registro'   => $this->fechaActual(),
                                                            'id_usuario'       => $this->session->userdata('idPersonaSession'),
    ...
    public function sendDataToSiom($idEECC, $idEstacion, $estacion_desc, $itemplan, $emplazamiento_id, $ptr){
        ...
        try{
            ...
            if($response->codigo == EXIT_SUCCESS){//SE CREO LA OS
                ...
                $this->m_liquidacion->insertLogTramaSiom($dataLogSiom);
