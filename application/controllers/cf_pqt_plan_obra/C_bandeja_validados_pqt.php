...
class C_bandeja_validados_pqt extends CI_Controller {

	function __construct(){
        parent::__construct();
        $this->output->set_header('Content-Type: text/html; charset=ISO-8859-1');
        $this->load->model('mf_pqt_plan_obra/m_bandeja_validados_pqt');
    ...
    public function index(){
	    ...
	    if($logedUser != null){
			/*
               ...
               $data['tablaData']     = $this->getTablaHojaGestion($this->m_bandeja_validados_pqt->getObrasToBandejaEspera());               
               $data['nombreUsuario'] =  $this->session->userdata('usernameSession');
               $data['perfilUsuario'] =  $this->session->userdata('descPerfilSession');
               $permisos =  $this->session->userdata('permisosArbol');
        	   $result = $this->lib_utils->getHTMLPermisos($permisos, 250, 294, ID_MODULO_ADMINISTRATIVO);
        	   $data['opciones'] = $result['html'];
        	   if($result['hasPermiso'] == true){
        	         $this->load->view('vf_pqt_plan_obra/v_bandeja_validados_pqt',$data);
        	   }else{
    ...
    function filtrarTabaDPS() {
        ...
        try {
            ...
            $data['tablaBandejaHG'] = $this->getTablaHojaGestion($this->m_bandeja_validados_pqt->getBandejaPepSisego($pep1));
