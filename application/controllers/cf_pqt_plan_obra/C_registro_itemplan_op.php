 ...
 *
 */
class C_registro_itemplan_op extends CI_Controller {
    ...
    public function index() {
        ...
        if ($logedUser != null) {
            ...
            ;
            $data['cmbItemplanMadre'] = __buildCmbItemplanMadre(4, null);

            $permisos = $this->session->userdata('permisosArbol');
            // permiso para registro individual modificar
            $result = $this->lib_utils->getHTMLPermisos($permisos, 298, 284, 2);
            $data['opciones'] = $result['html'];
            $this->load->view('vf_pqt_plan_obra/v_registro_itemplan_op', $data);
        } else {
...
//            $cantFactorMedicion = $this->input->post('inputCantidadFactorMedicion');
            ...
            $costoMo = 0;
            $itemMadre = $this->input->post('itemplanMadre');

            $dataMadre = $this->m_planobra->getItemplanMadre($itemMadre);
			
			if($dataMadre== null){
                throw new Exception('No se pudo obtener Informacion del Itemplan Madre');
            }            
            
            if($dataMadre['idEmpresaColab']== null){
                throw new Exception('No se detecto EECC en el Itemplan Madre!');
            }
                
            if($dataMadre['idEmpresaColab'] != $eecc){
                throw new Exception('La EECC del Itemplan hijo debe ser igual al del Itemplan Madre.');
            }
            ...
            if ($itemMadre == null) {
                throw new Exception('Debe seleccionar el itemplan Madre.');
            }

            $estadoplan = ESTADO_PLAN_PRE_REGISTRO;

            $this->m_planobra->deleteLogImportPlanObraSub();
            $itemplan = $this->m_planobra->generarCodigoItemPlan(ID_PROYECTO_OBRA_PUBLICA, $idzonal);
...
//            print_r($dataMadre);
            ...
            if ($data['error'] == EXIT_ERROR) {
                ...
                $dataArray = array(
                    ...
                    'descripcion' => utf8_decode('SE PASA DE PRE REGISTRO A DISEÑO'),
                    'idEstadoPlan' => ID_ESTADO_DISENIO,
                    //'solicitud_oc' => $dataMadre['solicitud_oc'],
//                    'orden_compra' => $dataMadre['orden_compra'],
//                    'posicion' => $dataMadre['posicion'],
//                    'cesta' => $dataMadre['cesta'],
//                    'estado_sol_oc' => $dataMadre['estado_sol_oc']
                );
                $data = $this->m_planobra->updateIP($dataArray, $itemplanData);
                /////////
    ...
    function createPlanObraFromSisego() {
        ...
        try {
            ...
            $tipo_cliente = utf8_decode($this->utf8input->post('tipo_cliente'));
            ...
            $data = $this->m_planobra->insertarPlanobra($itemplan, $idProy, $idSubproy, $idCentral, $idzonal, $eecc, $eelec, $estadoplan, $fase, $fechaInicio, $nombreplan, $indicador, $uip, $cordx, $cordy, $cantidadTroba, $has_coti, null, $tipo_requerimiento, $tipo_diseno, $nombre_estudio, $duracion, $acceso_cliente, $tendido_externo, $tipo_sede, $tipo_cliente, $per);
            if ($data['error'] == EXIT_ERROR) {
        ...
        try {
            ...
            if ($estado == 1) {
                ...
                if ($datosCluster != null) {
                    ...
                    $data = $this->m_planobra->insertarPlanobraCluster($itemplan, $idProyecto, $idSubProyecto, $idCentral, $idzonal, $idEmpresaColab, $eelec, $estadoplan, $fase, $fechaInicio, $nombreplan, $indicador, $uip, $cordx, $cordy, $cantidadTroba, $has_coti, $fechaCreacion, $fecPrevistaEjec);
    ...
    function getItemplanMadreFactorMed() {
        ...
        try {
            ...
            $data['cmbItemMadre'] = __buildCmbItemplanMadre(null, $idSubProyecto);
    ...
    // public function getCmbItemplanMadre(){
    //     $data['error']    = EXIT_ERROR;
    //     $data['msj']      = null;
    //     $data['cabecera'] = null;
    //     try{
    //         $idProyecto    = $this->input->post('proyecto');
    //         $idSubProyecto = $this->input->post('idSubProyecto');
    //         $data['cmbItemMadre'] = __buildCmbItemplanMadre($idProyecto, $idSubProyecto);
    //         $data['error']    = EXIT_SUCCESS;
    //     }catch(Exception $e){
    //         $data['msj'] = $e->getMessage();
    //     }
