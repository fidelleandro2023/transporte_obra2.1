...
class C_planobra extends CI_Controller {
	...
	function __construct(){
        ...
        $this->load->model('mf_pqt_pre_diseno/m_pqt_bandeja_adjudicacion');
	...
	public function index()
	{
	    ...
	    if($logedUser != null){
               ...
               /***$data['listaProy'] = $this->m_utils->getAllProyecto();***/
                $data['listaProy'] = $this->m_utils->getAllProyectoByNotId();
               /****************************************/
     ...
     */
     public function createPlanobra(){
        ...
        try{
            ...
            /**
             * nueva regla para no derivar obras a ezentis o liteyca
             * 
             */
			if($idProy  ==  ID_PROYECTO_OBRA_PUBLICA){
				if(in_array($eecc,array(ID_EECC_LITEYCA, ID_EECC_EZENTIS))){
					$eecc = 12;//PONEMOS COMO EECC A TDP EECC 19.07.2020
				}
			}
            /**
             * hasta aqui nueva regla para no derivar obras a ezentis o liteyca
             *
             */
			$pepGlobalMoviles = null;//czavala 06.07.2020
            $monto_tmp_final = null;//czavala 06.07.2020
			
            $idTipoFactorMedicion = $this->input->post('hfIdFactorMedicion');
            $cantFactorMedicion = $this->input->post('inputCantidadFactorMedicion');
            
            $hfAdjudicacionAutomatica = $this->input->post('hfAdjudicacionAutomatica');
            $hfHasCoaxial = $this->input->post('hfHasCoaxial');
            $hfHasFo = $this->input->post('hfHasFo');
            $hfpaquetizado_fg = $this->input->post('hfpaquetizado_fg');
            
            if($hfpaquetizado_fg == ID_MODULO_GESTION_OBRA){
                $idTipoFactorMedicion = null;
            }
			...
			$costoMo = null;
			if($idProy == ID_PROYECTO_OBRA_PUBLICA) {//SI ES OP EL COSTO MO ES EL QE INGRESAN EN FORMULARIO
                $costoMo  = $this->input->post('inputDiseno');
                if($costoMo == null || $costoMo == '') {
                    throw new Exception("Ingresar el costo de diseño");
                }
            }
			/**czavala 06.07.2020 moviles**/
            if($idProy==ID_PROYECTO_MOVILES && $has_coti !=  '1'){
                $hasSomePep = false;
                $jefatura = $this->m_utils->getJefaturaXCentral($idCentral);
                $tipoJefatura = (($jefatura=='LIMA') ? '1' : '2');
                $monto_pqt = $this->m_utils->getCostoPqtForFactorMedicionObra(1, ID_ESTACION_FO, $idSubproy, $eecc, $tipoJefatura);
                $itemBolsaPep = $this->m_utils->getPEPSBolsaPepBySubProyecto($idSubproy);
                if(count($itemBolsaPep)>0){
                    foreach ($itemBolsaPep as $pep){log_message('error','pep:'.$pep->pep1);
                        if($pep->monto_temporal >= $monto_pqt){
                            $hasSomePepWiPresu = true;
                            $pepGlobalMoviles = $pep->pep1;
                            $monto_tmp_final = ($pep->monto_temporal - $monto_pqt);                            
                            $hasSomePep = true;
                            break;
                        }
                    }                    
                ...
                if(!$hasSomePep){
                    throw new Exception('El subproyecto no tiene una PEP con presupuesto disponible.');
                }
            }
           ...
           /**nuevo 06.07.2020 czavala**/
           if(in_array($idProy, array(ID_PROYECTO_MOVILES, ID_PROYECTO_TRANSPORTE, 43))){//43 = proyectos varios
               $estadoplan     = ESTADO_PLAN_PRE_REGISTRO;
           }
           if(in_array($idProy, array(ID_PROYECTO_TRANSPORTE, 43))){//43 = proyectos varios
               $has_coti = 1;
           }
		   /**termino 06.07.2020 czavala**/
            $this->m_planobra->deleteLogImportPlanObraSub();
            $itemplan=$this->m_planobra->generarCodigoItemPlan($idProy,$idzonal);
            
            $validarExceptionContratoBucle = $this->m_pqt_planobra->obtExceptionContratoBucle($eecc, $idSubproy);
            if($validarExceptionContratoBucle['count']>0){
                $hfpaquetizado_fg = null;
                $idTipoFactorMedicion = null;
                $cantFactorMedicion = null;
            }
			
			if($idCentral ==null || $idCentral == '') {
				throw new Exception('Error al Insertar central null');
			}
            
            $data = $this->m_planobra->insertarPlanobraPaquetizado($itemplan,$idProy, $idSubproy, $idCentral, $idzonal, $eecc, $eelec, $estadoplan, $fase, $fechaInicio, $nombreplan,$indicador ,$uip, $cordx, $cordy ,$cantidadTroba, $has_coti, $itemMadre,null,null,null,null,null,null,null,null,null,$idTipoFactorMedicion, $cantFactorMedicion, $hfpaquetizado_fg, $idUsuario, $costoMo);
...
/*
                if($hasAutoPlanEnObra == null) {//SI NO TIENE CONFIGURACION AUTO CREACION EN OBRA PREGUNTA SI TIENE ADJUDICACION AUTOMATICA EN CASO CONTRARIO NO SE ADJUDICARA DICHA OBRA.
					if($has_coti    !=  '1'){//SOLO SI NO TIENE ADJUDICACION ES DECIR NACE EN PRE DISENO PREGUNTAR LA AUTO ADJUDICACION
                        ...
                        $conEsta = $this->m_pqt_bandeja_adjudicacion->countFOAndCoaxByItemplan($itemplanData);
                    ...
                    $datoEnObra =   $this->m_planobra->changeEstadoEnAprobacion($itemplanData);                   
                }                
            }
            */
            // SI TIENE ADJUDICACION AUTOMATICA, PROCEDEMOS A GRABAR LOS DATOS DE ADJUDICACION AUTOMATICA
/*
            if($hasAutoPlanEnObra == null){
				if($hfpaquetizado_fg == ID_MODULO_PAQUETIZADO && $hfAdjudicacionAutomatica == 1 && !in_array($idProy, array(ID_PROYECTO_MOVILES, ID_PROYECTO_TRANSPORTE, 43))){
					if($hfHasCoaxial == 1 || $hfHasFo == 1){
						$idFechaPreAtencionCoax1    = $this->input->post('idFechaPreAtencionCoax');
						$idFechaPreAtencionFo1    = $this->input->post('idFechaPreAtencionFo');
						
						$this->session->set_userdata('has_fo', $hfHasFo);
						$this->session->set_userdata('has_coax', $hfHasCoaxial);
						
						$this->m_bandeja_adjudicacion->adjudicarItemplan($data['itemplannuevo'],$idSubproy,$idCentral,$eecc, $idFechaPreAtencionCoax1, $idFechaPreAtencionFo1);
					}
				}
			}
			*/
			/**nueva validacion moviles**/
				if($idProy==ID_PROYECTO_MOVILES && $has_coti    !=  '1'){//si no es cotizacion crear oc con paquetizado
				   $jefatura = $this->m_utils->getJefaturaXCentral($idCentral);
				   $tipoJefatura = (($jefatura=='LIMA') ? '1' : '2');
				   $monto_pqt = $this->m_utils->getCostoPqtForFactorMedicionObra(1, ID_ESTACION_FO, $idSubproy, $eecc, $tipoJefatura);
				   $codigo_solicitud = $this->m_utils->getNextCodSolicitud();
				   $dataPlanobra = array(  "itemplan"                   => $itemplanData,
					   "costo_unitario_mo"          => $monto_pqt,
					   "costo_unitario_mat"         => (($monto_pqt*60)/40),
					   "solicitud_oc"               => $codigo_solicitud,
					   "estado_sol_oc"              => 'PENDIENTE',
					   "costo_unitario_mo_crea_oc"  => $monto_pqt,
					   "costo_unitario_mat_crea_oc" => (($monto_pqt*60)/40)
				   );
				   ...
				   $solicitud_oc_creacion = array('codigo_solicitud'    => $codigo_solicitud,
					   ...
					   'plan'              =>  'COTIZACION',
					   'pep1'              =>  $pepGlobalMoviles,
					   'pep2'              =>  $pepGlobalMoviles.'-001',
					   'estatus_solicitud' => 'NUEVO',
					   'tipo_solicitud'    =>  1//creacion
				   );
				   
				   $item_x_sol = array('itemplan'            =>  $itemplanData,
				   ...
				   $dataCrearOc = $this->m_utils->crearOCByMontoPqt($dataPlanobra, $solicitud_oc_creacion, $item_x_sol, $dataSapDetalle);
				}
			}
        }catch(Exception $e){
    ...
    public function getHTMLChoiceZonal(){
        ...
        try {
            ...
            $listaEECCALL = $this->m_utils->getAllEECC();
            $idEECCselect = '';
            foreach ($listaEECCALL->result() as $row) {
            ...
            foreach ($listaEECC->result() as $row) {
                ...
                log_message('error', $row->idEmpresaColab);
            }
            $data['idEmpresaColab'] = $idEECCselect;
            $data['listaEECC'] = $html;
...
*/
   ...
   function createClusteFromSisego(){
    ...
    function getCentralPorCoordenadas() {
        $data['error']    = EXIT_ERROR;
        $data['msj']      = null;
        try{
            ...
            $Y = $this->input->post('Y');
            $X = $this->input->post('X');
            //log_message('error', 'getCentralPorCoordenadas '. $Y. ' ' .$X );
            $array = array(
                0 => $Y,
                1 => $X
            );
            //log_message('error', print_r($array, true));
            $central = $this->coordenadas_utils->getBuscarArea($array);
    ...
    public function getInfoSubProCoaxFo()
    {
        $data['error'] = EXIT_ERROR;
        $data['msj'] = null;
        ...
        try {
            ...
            $idSubPro = $this->input->post('idSupProyecto');
            $lista = $this->m_pqt_bandeja_adjudicacion->getSubProyectoConCoaxFO($idSubPro);
    
            $data['has_coaxial'] = 0;
            $data['has_fo'] = 0;
            $data['adjudicacionAutomatica_fg'] = 0;
            $data['paquetizado_fg'] = ID_MODULO_GESTION_OBRA;
            
            //log_message('error','$idSubPro '.$idSubPro);
            
            foreach($lista->result() as $row){
                //log_message('error','coaxial '.$row->coaxial);
                //log_message('error','fo '.$row->fo);
                $data['has_coaxial'] = $row->coaxial;
                $data['has_fo'] = $row->fo;
                $data['adjudicacionAutomatica_fg'] = $row->adjudicacionAutomatica_fg;
                $data['paquetizado_fg'] = $row->paquetizado_fg;
            }
            

            //log_message('error','getInfoSubProCoaxFo paquetizado_fg '.$data['paquetizado_fg']);
    ...
    function permitirFasesCantItemplan(){
        $data['error']    = EXIT_ERROR;
        $data['msj']      = null;
        try{
            ...
            $data['configuracion_fg'] = '';
            $data['permitir_continuar_fg'] = '';
            
            $list = $this->m_subproyecto_fases_cant_itemplan->getCantItemplanFasesByIdSubProyecto($idSubProyecto);
            $cant = 0;
            foreach ($list as $row){
                if($row->fase == $fase){
                    $data['configuracion_fg'] = 1;
					
                    if($row->itemsplan<$row->cantItemPlan){
                        $data['permitir_continuar_fg'] = 0;
                    }else{
                        $data['permitir_continuar_fg'] = 1;
                    }
					$cant ++;
                }
            }
			
			if($cant == 0){//si no tiene configurado la fase
				$data['configuracion_fg'] = 1;
				$data['permitir_continuar_fg'] = 1;
			}
            $data['error']    = EXIT_SUCCESS;
    ...
    function test_sisego(){
        $data['error']    = EXIT_ERROR;
        $data['msj']      = null;
        $this->load->view('vf_pqt_plan_obra/v_registro_sisego',$data);
    }
}