...
class C_bandeja_solicitud_oc_transporte extends CI_Controller {

	function __construct(){
        parent::__construct();
        $this->output->set_header('Content-Type: text/html; charset=ISO-8859-1');
        $this->load->model('mf_transporte/m_bandeja_solicitud_oc_transporte');
    ...
    public function index(){
	    ...
	    if($logedUser != null){
               ...
               #$data['tablaSiom']     = $this->getTablaHojaGestion($this->m_bandeja_solicitud_oc_transporte->getBandejaSolOC('','',''));      
               $data['tablaSiom']     = $this->getTablaHojaGestion(null);			   
               $data['nombreUsuario'] =  $this->session->userdata('usernameSession');
               $data['perfilUsuario'] =  $this->session->userdata('descPerfilSession');
               $permisos =  $this->session->userdata('permisosArbol');
        	   $result = $this->lib_utils->getHTMLPermisos($permisos, 309, 313, 8);
        	   $data['opciones'] = $result['html'];
        	   if($result['hasPermiso'] == true){
        	         $this->load->view('vf_modulo_transporte/v_bandeja_solicitud_oc_transporte',$data);
        	   }else{
                ...
                foreach($listaHojaGestion as $row){     
                    ...
                    if($row->estado == 1){//PENDIENTE
						if($row->tipo_solicitud == 1){
							$btnEnProceso = '<a data-hg="" data-hgtxt="'.$row->codigo_solicitud.'" href="getRegistroOcTransporte?sol='.$row->codigo_solicitud.'"><i title="buscar" style="color:#A4A4A4" class="zmdi zmdi-hc-2x zmdi-time-restore-setting"></i></a>';
    ...
    function getPtrByHGestionOCTransp() {
        $data['msj'] = null;
        ...
        try {
			...
			$listaEstado = $this->m_bandeja_solicitud_oc_transporte->getPtrsByHojaGestion($hojaGestion);
			...
			// }else if($tipo_solicitud == 2){//edicion oc
				// $listaEstado = $this->m_bandeja_solicitud_oc_transporte->getItemOrdenCompraEdicion($hojaGestion);
				// $tablaSiom =  $this->getTablaListaItemPlanOCEdicion($listaEstado, $estado);
			// }else if($tipo_solicitud == 3){//edicion oc
				// $listaEstado = $this->m_bandeja_solicitud_oc_transporte->getItemOrdenCompraCerti($hojaGestion);
				// $tablaSiom =  $this->getTablaListaItemPlanOCCertificacion($listaEstado, $estado);
			// }else if($tipo_solicitud == 4){//anulacionPos oc
				// $listaEstado = $this->m_bandeja_solicitud_oc_transporte->getItemOrdenCompraAnulaPosOC($hojaGestion);
    ...
    function filtrarTabaCOCTransp() {
        ...
        try {
            ...
            $data['tablaBandejaHG'] = $this->getTablaHojaGestion($this->m_bandeja_solicitud_oc_transporte->getBandejaSolOC($solicitud,$itemplan,$estado));
            $data['error'] = EXIT_SUCCESS;
        } catch(Exception $e) {
            $data['msj'] = $e->getMessage();
        }
	...
	function valSolEdiOCTransporte() {
        $data['msj'] = null;
        $data['error'] = EXIT_ERROR;
        try {
			$this->db->trans_begin();
			...
			if($idPersona != null){
                    ...
                    $cod_solicitud_certi = $this->m_bandeja_solicitud_oc_transporte->getSolCertPndEdicion($codigo_solicitud);
                    
                    $dataSolCert = array('codigo_solicitud' => $cod_solicitud_certi,
                                         'estado'   =>  5//pnd de acta
                                            );
                    
                    $data = $this->m_bandeja_solicitud_oc_transporte->update_solicitud_ocV2($codigo_solicitud, $arrayData, $dataSolCert);
					
					if($data['error'] == EXIT_SUCCESS) {
						$data = $this->m_bandeja_solicitud_oc_transporte->actualizarMontoSap($codigo_solicitud, $costoSap);
    ...
    function setHojaGestionCertificado() {
        ...
        try {
            ...
            $idPersona           = $this->session->userdata('idPersonaSession');
            if($idPersona != null){
                $infoHg = $this->m_bandeja_solicitud_oc_transporte->getBolsaHgDataByHG($idHojaGestion);
                if($infoHg['estado']==3){
                    ...
                    if($arrayFile!=null){//si al menos viene 1 ptr en la hoja de gestion.
                       ...
                       */ 
                        $data = $this->m_bandeja_solicitud_oc_transporte->updateHojaGestionCertificacion($arraylogPlanobraPoData, $arrayUpCertificacionMo, $arrayUpPlanobraPo, $arrayUpDetallePlan, $dataHojagestion, $idHojaGestion);
                    }else{
	...
	public function makeCSVHojaGestionMO(){
        ...
        try{
            ...
            $detalleplan = $this->m_bandeja_solicitud_oc_transporte->getDataToExcelReport();
    ...
    function removeOnePtrFromHojaGestionTransp() {
        $data['msj'] = null;
        $data['error'] = EXIT_ERROR;
        try {
            ...
            $data = $this->m_bandeja_solicitud_oc_transporte->updatePtrFromHojaGestion($ptr, $arrayData, $idHojaGestion);
	...
	function certificarSolicitudOcTransp() {
		$data['msj'] = null;
        $data['error'] = EXIT_ERROR;
        try {
            ...
            if($idPersona != null){
                    ...
                    $data = $this->m_bandeja_solicitud_oc_transporte->update_solicitud_oc($codigo_solicitud, $arrayData);
    ...
    function validarAnulacionOcTransp() {
		$data['msj'] = null;
        $data['error'] = EXIT_ERROR;
        try {
	...
	function getActaCertificacion(){
        ...
        $dataSolicitud = $this->m_bandeja_solicitud_oc_transporte->getDataSolicitudPdf($cod_solicitud);
