...
class C_porcentaje_transporte extends CI_Controller {
    ...
    public function index(){
        ...
        $this->load->view('vf_modulo_transporte/v_porcentaje_transporte',$data);
    ...
    function ejecutarPorcentaje() {
        ...
        try {
            ...
            $idCuadrilla         = $this->input->post('select_cuadrilla');
    ...
    function formPorcentajeHTML($itemPlan, $idZonal, $idEstacion, $descEstacion) {
       ...
       // $head_2 = array('data' => 'Contrata'  , 'class' => 'text-center');
        ...
        $this->table->set_heading($head_0, $head_1, $head_3, $head_4, $head_5);
        ...
        if(in_array($idZonal, array(8,9,10,11,12)) && $racti->result()) {
            foreach($arrayLisAct as $row) {
              ...
              // $row_2 = array('data' => 'PRUEBA');
                ...
                $row_5 = array('data' => $btnActualizar." ".$btnDetalle."".$camara." ".$regCuad);
                $html = null;
                $this->table->add_row($row_0,$row_1, $row_3, $row_4, $row_5);
            }
        } else {
            ...
            $row_3 = array('data' => $html2);
            $row_4 = array('data' => $texArea);
            $row_5 = array('data' => $btnActualizar." ".$regCuad);
            $this->table->add_row($row_0, $row_1, $row_3, $row_4, $row_5);   
        }
        ...
        try{
           ...
           if($accion  ==  CREAR_REGISTRO){
                ...
                if($cambio){
                    ...
                    $listaPartidas      =   $this->M_porcentaje->getPartidasByItemplan($itemplan);
    ...
    function getTablaConsultaPTR($itemplan, $idEstadoPlan) {
        ...
        foreach($arrayPtr as $row) {
            //if($idEstadoPlan == ID_ESTADO_TERMINADO) {
              //  $btnEditar = null;
            //} else {
                $btnEditar = '<input type="button" class="btn-danger" data-costo_mo="'.$row->costo_mo.'" data-ptr="'.$row->ptr.'" data-itemplan="'.$itemplan.'" value="editar" onclick="openModalEditarPTR($(this));"/>';                
            //}
            $tb .= '<tr>
                        <td>'.$row->ptr.'</td>
                        <td>'.$row->total.'</td>
    ...
    function getPtrEditar() {
        ...
        foreach($arrayData as $row) {
            ...
            $html.= '<tr id="'.$cont.'">
                        ...
                        <td id="precioKit_'.$cont.'">'.$row->costo_material.'</td>
                        <td id="costoMA_'.$cont.'">'.$row->costo_mat.'</td>
