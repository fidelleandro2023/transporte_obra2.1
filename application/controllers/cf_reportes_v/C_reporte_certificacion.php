...
class C_reporte_certificacion extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($logedUser != null) {
            ...
            $arrayReporte = $this->m_extractor->getReporteCertCVByJefatura1();
            $data['tablaReporte'] = $this->makeHTLMTablaReporte($arrayReporte);

            $data['nombreUsuario'] = $this->session->userdata('usernameSession');
            $data['perfilUsuario'] = $this->session->userdata('descPerfilSession');
            $permisos = $this->session->userdata('permisosArbol');
            #$result = $this->lib_utils->getHTMLPermisos($permisos, ID_PERMISO_PADRE_CERTIFICACION_MO, ID_PERMISO_HIJO_REPORTE_CERTI_CV_MO);
            $result = $this->lib_utils->getHTMLPermisos($permisos, 250, ID_PERMISO_HIJO_REPORTE_CERTI_CV_MO, ID_MODULO_ADMINISTRATIVO);
            $data['opciones'] = $result['html'];
            if ($result['hasPermiso'] == true) {
                $this->load->view('vf_reportes_v/v_reporte_certificacion', $data);
            } else {
    ...
    {
        $html = '<table style="font-size: 10px;" id="reporte2" class="table table-bordered">
                    <thead class="thead-default">
                        <tr role="row">
                            <th colspan="2" rowspan="2" style="text-align: center;"></th>
                            <th colspan="1" rowspan="2" style="text-align: center;"></th>
    ...
    public function filtrarTabla()
    {
        ...
        try {
            ...
            $eecc = $this->input->post('eecc') ? $this->input->post('eecc') :null;
            $idFase = $this->input->post('idFase') ? $this->input->post('idFase') : null;
            $data['tablaReporte1'] = $this->makeHTLMTablaReporte($this->m_extractor->getReporteCertCVByJefatura1($proyecto,$jefatura,$eecc,$idFase));
            $data['error'] = EXIT_SUCCESS;
        } catch (Exception $e) {
            $data['msj'] = $e->getMessage();
        }
