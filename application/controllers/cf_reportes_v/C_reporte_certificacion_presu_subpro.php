...
class C_reporte_certificacion_presu_subpro extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($logedUser != null) {
            ...
            $arrayReporte = $this->m_extractor->getReporteCertCVByJefatura3();
            $data['tablaReporte'] = $this->makeHTLMTablaReporte2($arrayReporte);

            $data['nombreUsuario'] = $this->session->userdata('usernameSession');
            $data['perfilUsuario'] = $this->session->userdata('descPerfilSession');
            $permisos = $this->session->userdata('permisosArbol');
            #$result = $this->lib_utils->getHTMLPermisos($permisos, ID_PERMISO_PADRE_CERTIFICACION_MO, ID_PERMISO_HIJO_REPORTE_CERTI_CV_MO_PRESU_SUBPRO);
            $result  = $this->lib_utils->getHTMLPermisos($permisos, 250, ID_PERMISO_HIJO_REPORTE_CERTI_CV_MO_PRESU_SUBPRO, ID_MODULO_ADMINISTRATIVO);
            $data['opciones'] = $result['html'];
            if ($result['hasPermiso'] == true) {
                $this->load->view('vf_reportes_v/v_reporte_certificacion_presu_subpro', $data);
            } else {
    ...
    public function getReport2CertMO()
    {
        ...
        try {
            ...
            $data['tablaReporte2'] = $this->makeHTLMTablaReporte2($this->m_extractor->getReporteCertCVByJefatura3($proyecto,$jefatura,$eecc,$idFase));
            $data['error'] = EXIT_SUCCESS;
    ...
    public function makeHTLMTablaReporte2($arrayReporte)
    {
        $html = '<table style="font-size: 10px;" id="data-table" class="table table-bordered">
                    <thead class="thead-default">
                        ...
                        <tr role="row">
                            <th colspan="1" style="text-align: center;">SUBPROYECTO</th>
                            <th colspan="1" style="text-align: center;">EXPEDIENTE</th>
        ...
        foreach ($arrayReporte as $row) {
            ...
            $html .= '<tr>
                            ...
                            <td colspan="1" style="text-align:center; background-color: #ccccb3;">' . $row->subproyecto_ve . '</td>
                            <td colspan="1" style="text-align:center">SI</td>
    ...
    public function filtrarTabla()
    {
        ...
        try {
            ...
            $eecc = $this->input->post('eecc') ? $this->input->post('eecc') :null;
            $idFase = $this->input->post('idFase') ? $this->input->post('idFase') : null;
            $data['tablaReporte1'] = $this->makeHTLMTablaReporte2($this->m_extractor->getReporteCertCVByJefatura3($proyecto,$jefatura,$eecc,$idFase));
            $data['error'] = EXIT_SUCCESS;
        } catch (Exception $e) {
            $data['msj'] = $e->getMessage();
        }
