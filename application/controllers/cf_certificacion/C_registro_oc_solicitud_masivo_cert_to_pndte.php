...
class C_registro_oc_solicitud_masivo_cert_to_pndte extends CI_Controller {

    function __construct() {
        parent::__construct();
        $this->output->set_header('Content-Type: text/html; charset=ISO-8859-1');
        $this->load->model('mf_certificacion/m_registro_oc_solicitud_masivo_cert_to_pndte');
    ...
    public function index() {
        ...
        if ($logedUser != null) {
            ...
            if ($result['hasPermiso'] == true) {
                $this->load->view('vf_certificacion/v_registro_oc_solicitud_masivo_cert_to_pndte', $data);
            } else {
    ...
    public function getExcelPartidasMO() {
        ...
        try {
                    ...
                    $contador = 1;
                    $titulosColumnas = array('CODIGO SOLICITUD');
    ...
    public function makeHTMLBodyTable($row_dimension, $objPHPExcel) {
        ...
        for ($i = 1; $i <= $row_dimension['row']; $i++) {//COMIENZA DESDE LA FILA 1
            ...
            $total = 0;
            if ($A != '' && $A != 'CODIGO SOLICITUD') {
                $infoSol = $this->m_registro_oc_solicitud_masivo_cert_to_pndte->getInfoSolicitudOCCreaByCodigo(trim($A));
                log_message('error', print_r($infoSol, true));
                if($infoSol != null){
                    if($infoSol['estado']==5 && $infoSol['cant'] == 1){
                        ...
                        $pre_array = array($A, $infoSol['itemplan'], $infoSol['idEstadoPlan'], $infoSol['idSubProyecto'], $infoSol['idTipoPlanta'], $infoSol['paquetizado_fg']);
                        ...
                        if($infoSol['cant']==0 || $infoSol['cant']==null){
                            ...
                            $msj = 'SOLICITUD SE ENCUENTRA PDT DE EDICION';
						}
                    ...
                    if ($arrayFile != null) {
                        ...
                        foreach ($arrayFile as $datos) {
                            if ($datos != null) {
								...
								$dataSolicitud['fecha_to_pndte']    = $this->fechaActual();
                        ...
                        $data = $this->m_registro_oc_solicitud_masivo_cert_to_pndte->masiveUpdateAtencionCreateSolOC($solicitudesList);                       
                    } else {
