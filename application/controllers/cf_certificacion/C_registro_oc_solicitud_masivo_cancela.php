...
class C_registro_oc_solicitud_masivo_cancela extends CI_Controller {
    ...
    public function index() {
        ...
        if ($logedUser != null) {
            ...
            if ($result['hasPermiso'] == true) {
                $this->load->view('vf_certificacion/v_registro_oc_solicitud_masivo_cancela', $data);
            } else {
    ...
    public function getExcelPartidasMO() {
        ...
        try {
                    ...
                    $contador = 1;
                    $titulosColumnas = array('CODIGO SOLICITUD', 'MOTIVO');
    ...
    public function makeHTMLBodyTable($row_dimension, $objPHPExcel) {
        ...
        for ($i = 1; $i <= $row_dimension['row']; $i++) {//COMIENZA DESDE LA FILA 1
            ...
            if ($B != '' && $A != 'CODIGO SOLICITUD') {
                $infoSol = $this->m_registro_oc_solicitud_masivo_cancela->getInfoSolicitudOCCreaByCodigo(trim($A));
                if($infoSol != null){
                    if($infoSol['estado']==1){
                    ...
                    if ($arrayFile != null) {
                        ...
                        foreach ($arrayFile as $datos) {
                            if ($datos != null) {
								...
								$dataSolicitud['estado']            = 3;//cancelado
								$dataSolicitud['usuario_cancela']   = $idUsuario;
								$dataSolicitud['motivo_cancela']    = $motivo;		
								$dataSolicitud['fecha_cancelacion'] = $this->fechaActual();	
								
								array_push($solicitudesList, $dataSolicitud);
                            }
                        }
                      
                        $data = $this->m_registro_oc_solicitud_masivo_cancela->masiveUpdateAtencionCreateSolOC($solicitudesList);                       
                    } else {
