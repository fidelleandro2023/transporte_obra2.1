...
class C_registro_oc_solicitud_masivo_dev extends CI_Controller {
    ...
    public function index() {
        ...
        if ($logedUser != null) {
            ...
            if ($result['hasPermiso'] == true) {
                $this->load->view('vf_certificacion/v_registro_oc_solicitud_masivo_dev', $data);
            } else {
    ...
    public function makeHTMLBodyTable($row_dimension, $objPHPExcel) {
        ...
        for ($i = 1; $i <= $row_dimension['row']; $i++) {//COMIENZA DESDE LA FILA 1
            ...
            if ($B != '' && $C != '' && $A != 'CODIGO SOLICITUD') {
                $infoSol = $this->m_registro_oc_solicitud_masivo_dev->getInfoSolicitudOCCreaByCodigo(trim($A));
				$costo_resta = intval(abs($infoSol['costo_unitario_mo'] - $C));
				// _log(print_r($infoSol, true));
				// _log("costo: ".$costo_resta);
				// _log("costo1: ".$infoSol['costo_unitario_mo']);
				// _log("costo2: ".$C);
                    ...
                    if ($arrayFile != null) {
                        ...
                        foreach ($arrayFile as $datos) {
                            if ($datos != null) {
								...
								$cod_solicitud_certi = $this->m_registro_oc_solicitud_masivo_dev->getSolCertPndEdicion($codigo_solicitud);
                        ...
                        $data = $this->m_registro_oc_solicitud_masivo_dev->masiveUpdateAtencionCreateSolOC($itemplanList, $solicitudesList, $solCertis);                       
                    } else {
