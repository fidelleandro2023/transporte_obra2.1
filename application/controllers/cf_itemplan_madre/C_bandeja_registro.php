...
class C_bandeja_registro extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('mf_itemplan_madre/m_bandeja_registro');
    ...
    function index() {
        ...
        if ($logedUser != null) {
            ...
            $result = $this->lib_utils->getHTMLPermisos($permisos, 298, 285, 2);
            $data['opciones'] = $result['html'];

            $this->load->view('vf_itemaplan_madre/v_bandeja_registro', $data);
        } else {
    ...
    function getPepItemplanMadre() {
        ...
        try {
            ...
            $result = $this->m_bandeja_registro->getPepItemplanMadre($this->input->post('cmbSubProyecto'));
            if (count($result) === 0) {
                throw new Exception('No encontro ninguna PEP');
            }
            foreach ($result as $row) {
                $pep = $this->m_bandeja_registro->getSAPdetalle($row['pep1']);
                foreach ($pep as $key) {
                    if ($count == 0) {
    ...
    function getTbItemsMadre($ItemplanmMadre) {
        ...
        $cont = 0;
        $arrayData = $this->m_bandeja_registro->getDataTablaItemMadre($ItemplanmMadre);
//<td width="10%"><div><a><i title="VALIDAR DISE&Ntilde;O" style="color:#A4A4A4" class="zmdi zmdi-hc-2x zmdi-check-circle"></i></a>&nbsp;&nbsp;<a><i title="VALIDAR OPERACIONES" style="color:#A4A4A4" class="zmdi zmdi-hc-2x zmdi-check-circle"></i></a>&nbsp;&nbsp;<a onclick="modalItemplanHijos(' . "'" . $row['itemplan_m'] . "'" . ')"><i title="VER ITEMPLAN HIJOS" style="color:#A4A4A4" class="zmdi zmdi-hc-2x zmdi-eye"></i></a></div></td>
        ...
        foreach ($arrayData as $row) {
            ...
            $prioridad = '';
            $descarga = '';
            $gestion = '&nbsp;&nbsp;<a><i title="Gestion" style="color:#A4A4A4" class="zmdi zmdi-hc-2x zmdi-city-alt"></i></a>';
            ...
            $html .= '<tr>
                                        <td width="15%"><div>' . $descarga . $prioridad . '<a onclick="modalItemplanHijos(' . "'" . $row['itemplan_m'] . "'" . ')"><i title="VER ITEMPLAN HIJOS" style="color:#A4A4A4" class="zmdi zmdi-hc-2x zmdi-eye"></i></a></div></td>
                                        <td>' . $row['itemplan_m'] . '</td>  
                                        <td>' . $this->m_bandeja_registro->cantidadItemplanHijos($row['itemplan_m']) . '</td>  
    ...
    public function hijosItemMadreDetalle($id) {
        $dataConsulta = $this->m_bandeja_registro->hijosItemMadreDetalle($id);
        $html = '';
        $totalMA = 0;
        $totalMo = 0;
    ...
    function montoToltalMA($id) {
        $montoToltal = $this->m_bandeja_registro->montoToltal($id);
        $total = 0;
    ...
    function updateConPrioridad() {
        ...
        try {
            ...
            $countItemplanMadre = $this->m_bandeja_registro->countItemplanMadre($itemplanM);

            if ($countItemplanMadre == 0) {
                $data = $this->m_bandeja_registro->saveDetalleObraPublica($objRegDetalle);
            } else {
                $data = $this->m_bandeja_registro->updateConPrioridadDetalle($objRegDetalle, $itemplanM);
            }

            if ($data['error'] == EXIT_ERROR) {
                throw new Exception('Error al Actualziar Itemplan Madre');
            } else {
                $this->m_bandeja_registro->updateConPrioridad($objReg, $itemplanM);
            }
        } catch (Exception $e) {
            $data['msj'] = $e->getMessage();
        }
    ...
    function getEditItemplanMadre() {
        ...
        try {
            $id = $this->input->post('itemplanMadre');
            $data['datos'] = $this->m_bandeja_registro->getEditItemplanMadre($id);
            $data['error'] = EXIT_SUCCESS;
        } catch (Exception $e) {
            $data['msj'] = $e->getMessage();
        }
