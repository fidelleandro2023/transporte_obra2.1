...
class C_bandeja_validacion extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('mf_itemplan_madre/m_bandeja_validacion');
    ...
    function index() {
        ...
        if ($logedUser != null) {
            ...
            $result = $this->lib_utils->getHTMLPermisos($permisos, 298, 299, 2);
            $data['opciones'] = $result['html'];

            $this->load->view('vf_itemaplan_madre/v_bandeja_validacion', $data);
        } else {
    ...
    function getPepItemplanMadre() {
        ...
        try {
            ...
            $result = $this->m_bandeja_validacion->getPepItemplanMadre($this->input->post('cmbSubProyecto'));
            if (count($result) === 0) {
                throw new Exception('No encontro ninguna PEP');
            }
            foreach ($result as $row) {
                $pep = $this->m_bandeja_validacion->getSAPdetalle($row['pep1']);
                foreach ($pep as $key) {
                    if ($count == 0) {
    ...
    function getTbItemsMadre($ItemplanmMadre) {
        ...
        $cont = 0;
        $arrayData = $this->m_bandeja_validacion->getDataTablaItemMadre($ItemplanmMadre);
//<td width="10%"><div><a><i title="VALIDAR DISE&Ntilde;O" style="color:#A4A4A4" class="zmdi zmdi-hc-2x zmdi-check-circle"></i></a>&nbsp;&nbsp;<a><i title="VALIDAR OPERACIONES" style="color:#A4A4A4" class="zmdi zmdi-hc-2x zmdi-check-circle"></i></a>&nbsp;&nbsp;<a onclick="modalItemplanHijos(' . "'" . $row['itemplan_m'] . "'" . ')"><i title="VER ITEMPLAN HIJOS" style="color:#A4A4A4" class="zmdi zmdi-hc-2x zmdi-eye"></i></a></div></td>
        ...
        foreach ($arrayData as $row) {
            ...
            $html .= '<tr>
                                        <td><div><a onclick="modalItemplanHijos(' . "'" . $row['itemplan_m'] . "'" . ')"><i title="VER ITEMPLAN HIJOS" style="color:#A4A4A4" class="zmdi zmdi-hc-2x zmdi-check-all"></i></a></div></td>
                                        <td>' . $row['itemplan_m'] . '</td>  
                                        <td>' . $this->m_bandeja_validacion->cantidadItemplanHijos($row['itemplan_m']) . '</td>  
                                        <td>' . utf8_decode($row['nombre']) . '</td>
                                        <td>' . utf8_decode(mb_strtoupper($row['proyectoDesc'])) . '</td>
                                        <td>' . $row['subDesc'] . '</td>
                                        <td>' . $row['prioridad'] . '</td>
                                        <td>' . ' s./ ' . $row['costoEstimado'] . '</td>
    ...
    public function hijosItemMadreDetalle($id) {
        $dataConsulta = $this->m_bandeja_validacion->hijosItemMadreDetalle($id);
        $html = '';
        $totalMA = 0;
        $totalMo = 0;
        if (count($dataConsulta) == 0) {
            ...
            foreach ($dataConsulta as $row) {
                ...
                if ($row->estado_di == 2) {
                    $dise = '<div title="Aprobado" disabled class="text-center" ><i class="zmdi zmdi-hc-2x zmdi-check" style="color:green"></i></div>';
                } else if ($row->estado_di == 3) {
                    $dise = '<div title="Rechazado" disabled class="text-center" ><i class="zmdi zmdi-hc-2x zmdi-close" style="color:red"></i></div>';
                } else {
                ...
                if ($row->estado_ope == 2) {
                    $ope = '<div title="Aprobado" class="text-center" ><i class="zmdi zmdi-hc-2x zmdi-check" style="color:green"></i></div>';
                } else if ($row->estado_ope == 3) {
                    $ope = '<div title="Rechazado" class="text-center" ><i class="zmdi zmdi-hc-2x zmdi-close" style="color:red"></i></div>';
                } else {
...
//                <td><input checked class="custom-checkbox" type="checkbox" value="verde" id="input-verde" name="coloresPreferidos"><label for="input-verde"></label></td>
    ...
    function montoToltalMA($id) {
        $montoToltal = $this->m_bandeja_validacion->montoToltal($id);
        $total = 0;
    ...
    function updateConPrioridad() {
        ...
        try {
            ...
            $data = $this->m_bandeja_validacion->updateConPrioridadDetalle($objRegDetalle, $itemplanM);
            if ($data['error'] == EXIT_ERROR) {
                throw new Exception('Error al Actualziar Itemaplan Madre');
            } else {
                $this->m_bandeja_validacion->updateConPrioridad($objReg, $itemplanM);
            }
        } catch (Exception $e) {
            $data['msj'] = $e->getMessage();
        }
    ...
    function getEditItemplanMadre() {
        ...
        try {
            $id = $this->input->post('itemplanMadre');
            $data['datos'] = $this->m_bandeja_validacion->getEditItemplanMadre($id);
            $data['error'] = EXIT_SUCCESS;
        } catch (Exception $e) {
            $data['msj'] = $e->getMessage();
        }
            ...
            if ($data['error'] == EXIT_ERROR) {
                ...
                if ($idpep) {
                    if ($selectPrioridad === '1') {
                        _log('idpep : ' . $idpep);
                        $this->m_bandeja_validacion->OCregistroItemplanMadre($itemplanM, $idpep);
                    }
                }
            }
        } catch (Exception $e) {
    ...
    function validarItemplan() {
        ...
        try {
            ...
            $objReg = array(
                ...
                'idUsu' => $this->session->userdata('idPersonaSession'),
                'comentario' => $this->input->post('textComentario'),
                'fecha_registro' => $this->fechaActual(),
                $this->input->post('colum') => $this->input->post('selectEstado')
            );

            $count = $this->m_bandeja_validacion->cantidadItemplanVali($this->input->post('itemPlan'));

            if ($count == 0) {
                $data = $this->m_bandeja_validacion->validarItemplan($objReg);
            } else {
                $objReg2 = array(
                    $this->input->post('colum') => $this->input->post('selectEstado')
                );
                $data = $this->m_bandeja_validacion->UpdatevalidarItemplan($objReg2, $this->input->post('itemPlan'));
            }

            $number_ip = $this->m_bandeja_validacion->cantidadItemplanHijos($this->input->post('itemPlanPE'));
           
            if ($number_ip == 0) {
                $this->m_bandeja_validacion->updateItemplanMadre($this->input->post('itemPlanPE'));
            }
        } catch (Exception $e) {
            $data['msj'] = $e->getMessage();
        }
