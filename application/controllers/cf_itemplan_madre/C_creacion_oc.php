...
class C_creacion_oc extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('mf_itemplan_madre/m_creacion_oc');
    ...
    public function index() {
        ...
        if ($logedUser != null) {
            ...
            if ($result['hasPermiso'] == true) {
                $this->load->view('vf_itemaplan_madre/v_creacion_oc', $data);
            } else {
            ...
            foreach ($listaHojaGestion as $row) {
                ...
                if ($row->estado == 1) {//PENDIENTE
                    if ($row->tipo_solicitud == 1) {
                        $btnEnProceso = '<a data-hg="' . $row->id . '" data-hgtxt="' . $row->codigo_solicitud . '" href="reSolOcItMadre?sol=' . $row->codigo_solicitud . '"><i title="buscar" style="color:#A4A4A4" class="zmdi zmdi-hc-2x zmdi-time-restore-setting"></i></a>';
                        ...
                        $btnEnProceso = '<a data-costo_total="' . $row->costo_total . '" data-pep="' . $row->pep1 . '" data-hg="' . $row->id . '" data-codigo_sol="' . $row->codigo_solicitud . '" onclick="validarAnulacionOc($(this))">
                ...
                $html .= ' <tr>
                            <th style="width:7%">
                               <a data-idhg="' . $row->id . '" data-hg="' . $row->codigo_solicitud . ' "data-ts="' . $row->tipo_solicitud . '" onclick="getPtrByHojaGestion(this)"><i title="buscar" style="color:#A4A4A4" class="zmdi zmdi-hc-2x zmdi-search"></i></a> 
                                 ' . $btnEnProceso . '                        
                            </th>
    ...
    function getTablaListaItemPlanOCCertificacion($listaEstado, $estado) {
        ...
        foreach ($listaEstado as $row) {
            ...
            $indice++;
            array_push($array_valido, $row->solicitud_oc . '|' . $row->solicitud_oc);
        }
    ...
    function getTablaListarPtrByHojaGestion($listaEstado, $estado) {
        ...
        foreach ($listaEstado as $row) {
            ...
            $indice++;
            array_push($array_valido, $row->solicitud_oc . '|' . $row->itemplan_m);
        }
    ...
    function certificarSolicitudOc() {
        ...
        try {
            ...
            $itemplan = $this->m_creacion_oc->getItemplanCer($codigo_solicitud);
            ...
            if ($idPersona != null) {
                ...
                $this->m_creacion_oc->update_itemplan($arrayDataItem, $itemplan);
            } else {
                throw new Exception('Su sesion a terminado, Refresque la pantalla y vuelva a iniciar Sesion.');
            }
        } catch (Exception $e) {
    ...
    function validarSolicitudEdicionOC() {
        ...
        try {
            ...
            $itemplan = $this->m_creacion_oc->getItemplan($codigo_solicitud);
            $montoDev = $this->m_creacion_oc->getMontoDev($codigo_solicitud);
            $idPersona = $this->session->userdata('idPersonaSession');
            if ($idPersona != null) {
                $arrayData = array('estado' => 2,
                    'usuario_valida' => $this->session->userdata('idPersonaSession'),
                    'fecha_valida' => $this->fechaActual());
//                $costo_unitario_mo_anula = $this->m_creacion_oc->montoSolicitud($codigo_solicitud, $arrayData);

                $arrayDataItem = array(
                    'estado_oc_dev' => 'ATENDIDO',
                    'costoEstimado' => $montoDev
                );

                $data = $this->m_creacion_oc->update_solicitud_oc($codigo_solicitud, $arrayData);
                //$this->m_creacion_oc->update_costo_update($codigo_solicitud);
                $this->m_creacion_oc->update_itemplan($arrayDataItem, $itemplan);
            } else {
                throw new Exception('Su sesion a terminado, Refresque la pantalla y vuelva a iniciar Sesion.');
            }
    ...
    function validarAnulacionOc() {
        ...
        try {
            ...
            $idPersona = $this->session->userdata('idPersonaSession');
            $textMonto = $this->input->post('costo_total');
            $idpep = $this->input->post('pep');
            ...
            if ($idPersona != null) {
                ...
                $data = $this->m_creacion_oc->update_solicitud_oc($codigo_solicitud, $arrayData);
                $this->m_creacion_oc->updateIP($codigo_solicitud);
                $this->m_creacion_oc->updateMontoPEP($idpep, $textMonto);
            } else {
                throw new Exception('Su sesion a terminado, Refresque la pantalla y vuelva a iniciar Sesion.');
            }
        } catch (Exception $e) {
