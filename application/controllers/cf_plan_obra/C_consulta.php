...
class C_consulta extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($logedUser != null) {
            ...
            $this->load->view('vf_plan_obra/v_consulta', $data);
    ...
    public function makeHTLMTablaConsulta($listaPTR)
    {
        ...
        if ($listaPTR != '') {
            foreach ($listaPTR->result() as $row) {
				...
				if($countParalizados > 0 &&  ($idMotivoParaliza == 11 || $idMotivoParaliza == 66 || $countExitParaliza == 0)){
                    $btnParalizacion = '';
                }				
				/** fin nuevo 13.02.2020 **/
                ...
                if($row->idProyecto ==  ID_PROYECTO_OBRA_PUBLICA){
                    $btnGoGestionOP =   '<a href="gestionOP?item='.$row->itemPlan.'" target="_blank"><i style="color:#A4A4A4;cursor:pointer" class="zmdi zmdi-hc-2x zmdi-city-alt" title="Gestion Obra Publica"></i></a>';
                }
                ...
                $html .= '
                        <tr>
                              <td><div class="size">'.(($countParalizados> 0 ? '<b>PARALIZADO</b> <br>' : '')).$btnCvDoc.' '.$btnDatosSisego.' '.$iconsSiom.' '.$analisisEco.' '.$btnParalizacion.' '. (($row->idProyecto == ID_PROYECTO_SISEGOS) ? '<a href="https://gicsapps.com:8080/obras2/general/estudio_itemplan.php?itemplan='.$row->itemPlan.'" target="_blank"><i style="color:#A4A4A4" title="ver" class="zmdi  zmdi-hc-2x zmdi-eye"></i>'
                                   : '').' <a data-itemplan ="'.$row->itemPlan.'" onclick="zipItemPlan($(this));"><i style="color:#A4A4A4" title="evidencia" class="zmdi zmdi-hc-2x zmdi-folder-outline"></i></a>
                                                        <a data-itm='.$row->itemPlan.' onclick="openGant(this)"><i style="color:#A4A4A4" title="gantt" class="zmdi zmdi-hc-2x zmdi-input-svideo"></i></a>
                                                        <a data-idlog ="'.$row->itemPlan.'" onclick="mostrarLog(this)"><i title="buscar" style="color:#A4A4A4" class="zmdi zmdi-hc-2x zmdi-search"></i>
                                                        '.$btnGoGestionOP.$linkSisegosFormu.$btnLogOc.$btnDeleteOTAC.'</div></td> 
                            <td><a '.(($row->idEstadoPlan	==	ID_ESTADO_CERRADO || $row->idEstadoPlan	==	ID_ESTADO_CANCELADO) ?  'onclick="showMessageIsObraCerrada('.$row->idEstadoPlan.')"': 'href="' . (($row->idTipoPlanta == 1) ? 'detalleObra?item=' . $row->itemPlan . '&from=1' : 'detallePI?item=' . $row->itemPlan . '&from=1') . '"        target="_blank"').'							
							>' . $row->itemPlan . '</a></td>
    ...
    public function filtrarTabla()
    {
        ...
        try {
            ...
            $data['tablaAsigGrafo'] = $this->makeHTLMTablaConsulta($this->m_consulta->getPtrConsultaNoPqt($itemPlan, $nombreproyecto, $nodo, $zonal, $proy, $subProy, $estado, $filtroPrevEjec, $tipoPlanta,$idEECC, null, null, $idFase));
    ...
    public function gettHTMLMotivoCancel()
    {
        ...
        $logMotivoCancel = $this->m_log_ingfix->getMotivoTruncoCanceladoLog($this->session->flashdata('itemPlan'),
            $this->session->flashdata('fechaCancel'), 6);
        $htmlMotivoCancel = "";
    ...
    public function gettHTMLMotivoTrunco()
    {
        ...
        $logMotivoTrunco = $this->m_log_ingfix->getMotivoTruncoCanceladoLog($this->session->flashdata('itemPlan'),
            $this->session->flashdata('fechaTrunco'), 10);
        $htmlMotivoTrunco = "";
...
function getDataSiom() {
    ...
    public function getAllDataLogPlanObra2()
    {
        ...
        try {
			...
			$logParalizacion = $this->m_log_ingfix->getParalizadoObra($this->session->flashdata('itemPlan'));
			...
			if(count($logParalizacion->result()) > 0) {
				
				foreach ($logParalizacion->result() as $row) {
					$htmlParalizado .= '<tr class="item">
									   <td>PARALIZADO</td>
									   <td>' . $row->nombre . '</td>
									   <td>' . $row->fechaRegistro . '</td>
            ...
            $html = trim($htmlCabcer . $htmlCreacionIP . $htmlParalizado . $htmlAjud . $htmlDisenoEjecu . $htmlDisenoParcial . $htmlEnObra. 
