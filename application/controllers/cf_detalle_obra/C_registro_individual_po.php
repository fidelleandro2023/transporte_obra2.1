...
class C_registro_individual_po extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($logedUser != null) {
            ...
            $data['item'] = "'".$item."'";//cambio czavala
            $data['form'] = $est;
			$data['idEstacion'] = $idEstacion;
			$flg_paquetizado = $this->m_utils->getFlgPaquetizadoPo($item);
			
			if($flg_paquetizado == 2) {
				$dataArrayIP = $this->m_detalle_obra->getDetallePqtIP($item);
			} else {
				$dataArrayIP = $this->m_detalle_obra->getDetalleIP($item);
			}
            ...
            $this->load->view('vf_detalle_obra/v_registro_individual_po', $data);
        } else {
    ...
    public function cargarArchivoPO()
    {
        ...
        try {
			...
			if($flg_paquetizado == 2 || $flg_paquetizado == 1) {
				$dataArrayIP = $this->m_detalle_obra->getDetallePqtIP($itemplan);
			} else {
				$dataArrayIP = $this->m_detalle_obra->getDetalleIP($itemplan);
			}
			$idSubProyecto  = $dataArrayIP['idSubProyecto'];
			$idEmpresaColab = $dataArrayIP['idEmpresaColab'];
			
            if ($itemplan == null || $idEstacion == null || $idEmpresaColab == null || $idUsuario == null || $idSubProyecto == null) {
                throw new Exception('Su sesion ha expirado, refresque la pantalla y vuelva a intentarlo.');
            }
            ...
            foreach ($object->getWorksheetIterator() as $worksheet) {
                ...
                for ($row = 2; $row <= $highestRow; $row++) {
                    ...
                    $exitMatKit = $this->m_detalle_obra->countMatxKit($codMaterial, $idSubProyecto,$idEstacion);
                    $detalleMat = $this->m_detalle_obra->getDetalleMaterial($codMaterial, $idSubProyecto, $idEstacion);
                    $flgMatPhaseOut = $this->m_detalle_obra->countMatPhaseOut($codMaterial);
    ...
    public function registPO()
    {
        ...
        try {
			...
			if($flg_paquetizado == 2 || $flg_paquetizado == 1) {
				$infoEECC = $this->m_registro_po_mo->getEeccDisenoOperaByItemPlanPqt($itemplan);
				...
				if($countCienPor > 0) {
					throw new Exception('Estaci&oacute;n liquidad, ya no se puede crear m&aacute;s POs.');
				}
            ...
            if ($tm == 1) {
                if ($countCodPO >= 0 && $countCodPO <= 3) {
                    if ($costoTotalMat != null && $costoTotalMat > 0) {
                        $data = $this->m_detalle_obra->insertarPO($arrayInsertPO);
                    } else {
            ...
            if ($data['error'] == EXIT_SUCCESS) {
                foreach ($arrayMat as $row) {
                    ...
                    $exitMatKit = $this->m_detalle_obra->countMatxKit($row['codigo_material'], $idSubProyecto, $idEstacion);
                ...
                if (count($arrayInsertDetallePOGlob) > 0) {
                    $data = $this->m_detalle_obra->insertarDetallePO($arrayInsertDetallePOGlob);
                } else {
                    $data['error'] = EXIT_ERROR;
                ...
                if ($data['error'] == EXIT_SUCCESS) {
                    ...
                    $data = $this->m_detalle_obra->insertarLOGPO($arrayInsertLOGPO);
                    if ($data['error'] == EXIT_SUCCESS) {
                        ...
                        $data = $this->m_detalle_obra->insertarDetallePlan($arrayInsertDetallePlan);
                        if ($data['error'] == EXIT_SUCCESS) {
                                    ...
                                    if ($fechaPrevEjec != null) {
                                        ...
                                        if ($diferencia <= 60) {
                                            ...
                                            if ($data['error'] == EXIT_SUCCESS) {
                                                ...
                                                $data = $this->m_detalle_obra->insertarLOGPO($arrayInsertLOGPO2);
    ...
    public function getExcelPOMat()
    {
        ...
        try {
			...
			$itemplan = $this->session->userdata('itemplan_temp');
            ...
            $arrayMateriales = $this->m_detalle_obra->getMateriales_x_Material($dataArrayIP['idSubProyecto'], $idEstacion);
