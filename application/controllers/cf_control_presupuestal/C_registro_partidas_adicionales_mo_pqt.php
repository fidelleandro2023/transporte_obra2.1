...
class C_registro_partidas_adicionales_mo_pqt extends CI_Controller {

    function __construct(){
        parent::__construct();
        $this->output->set_header('Content-Type: text/html; charset=ISO-8859-1');
        $this->load->model('mf_control_presupuestal/m_registro_partidas_adicionales_mo_pqt');
    ...
    public function index(){
        ...
        if($logedUser != null){
            ...
            $data['codPo']          =   $codigo_po;
            $data['estacionDesc']   =   $this->m_utils->getEstaciondescByIdEstacion($idEstacion);
            $data['nombreUsuario']  =   $this->session->userdata('usernameSession');
            $data['perfilUsuario']  =   $this->session->userdata('descPerfilSession');
            $this->load->view('vf_control_presupuestal/v_registro_partidas_adicionales_mo_pqt',$data);            
    ...
    public function getExcelPartidasMO()
    {
        ...
        try {
            ...
            $codigo_po  = $this->input->post('cod_po');
            
            $arrayMateriales = $this->m_registro_partidas_adicionales_mo_pqt->getPartidasByProyectoEstacion($itemplan, $codigo_po);			
            if (is_array($arrayMateriales)) {
                ...
                if (count($arrayMateriales) > 0) {
                    ...
                    foreach ($arrayMateriales as $row) {
                        ...
                        ->setCellValue("F{$contador}", $row->cantidad_final);
                    }
    ...
    public function makeHTMLBodyTable($row_dimension, $objPHPExcel, $itemplan, $idEstacion){
        ...
        for ($i = 1; $i <= $row_dimension['row']; $i++){//COMIENZA DESDE LA FILA 1
            ...
            if($F!='' && $A!='CODIGO'){
                if(is_numeric($F)){
                    $infoPartida    =   $this->m_registro_partidas_adicionales_mo_pqt->getInfoCodigoMaterial($itemplan, $idEstacion, $A);
                    if($infoPartida!=null){
                         $D = $infoPartida['costo'];
                        /***********************************************/
    ...
    public function savePoMo(){
        ...
        try{
            ...
            if($idUsuario   !=     null){
                ...
                $cod_po         = $this->input->post('cod_po');
                $arrayFile = json_decode($jsonDataFile);
                $arrayFinalInsert = array();
                $arrayFinalInsert_tmp = array();
                $total_part_adicionales = 0;
                if($arrayFile!=null){
                                ...
                                if($datos[8]    ==  1){//registrar
                                    if(!in_array($datos[6], $arrayActividades)){
                                     ...
                                     // $dataCMO['idEstacion']       = $idEstacion;                                        
                                        ...
                                        $total_part_adicionales = $total_part_adicionales + $datos[7];
                    ...
                    $hasActivo = $this->m_utils->hasSolExceActivo($itemplan, null);
                    if($hasActivo>0){
                        throw new Exception('Accion no permitida, La obra ya cuenta con una solicitud de exceso PDT de Aprobacion.');                        
                    }                    
                    $infoCostosObra = $this->m_utils->getVariablesCostoUnitario($itemplan, TIPO_PO_MANO_OBRA, $cod_po);
                    ...
                    $costoUnitarioObra = $infoCostosObra['costo_unitario_mo'];
                    if($costoUnitarioObra==null || $costoUnitarioObra==0){
                        throw new Exception('La Obra no cuenta con Costo Unitario Registrado.');
                    }                    
                    $infoPo = $this->m_registro_partidas_adicionales_mo_pqt->getInfoPo($cod_po);
                    if($infoPo==null){
                    ...
                    $costoTotalPo           = $total_part_adicionales + $infoPo['monto_pqt'];                    	
                    ...
                    $data = $this->m_registro_partidas_adicionales_mo_pqt->createSolExcesoPartidasAdicionalesPqt($arrayFinalInsert, $dataSolExceso);
