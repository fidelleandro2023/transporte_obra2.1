...
class C_bandeja_certificacion_2 extends CI_Controller {
  
    function __construct(){
        parent::__construct();
        $this->load->model('mf_utils/m_utils');
        $this->load->model('mf_plantaInterna/m_bandeja_certificacion_2');     
    ...
    function index() {
        ...
        if($logedUser != null){
            ...
            $result   = $this->lib_utils->getHTMLPermisos($permisos, 247, ID_PERMISO_HIJO_BANDEJA_CERTIFICACION_II, ID_MODULO_ADMINISTRATIVO);
			$data['opciones'] = $result['html'];
            if($result['hasPermiso'] == true){
                $this->load->view('vf_plantaInterna/V_bandeja_certificacion_2',$data);
            }else{
    ...
    function getBandejaCertificacion($fechaIn, $fechaFin, $idEecc) {
        $arrayData = $this->m_bandeja_certificacion_2->getDataPlantaInterna(ID_ESTADO_TERMINADO, ID_TIPO_PLANTA_INTERNA, $fechaIn, $fechaFin, $idEecc);
        $btnTerminar = null;
    ...
    function getBandejaReporte($itemplan) {
        $arrayData = $this->m_bandeja_certificacion_2->getPartidasByItemplan($itemplan);
        $btnTerminar = null;
