 ...
 *
 */
class C_bandeja_cotizacion extends CI_Controller {
    ...
    function index(){
	    ...
	    if($logedUser != null){
               ...
               $data['tablaAsigGrafo'] = $this->makeHTLMTablaConsulta($this->m_consulta->getPtrConsulta('','','','','','',ID_ESTADO_PRE_DISENIO,'',ID_TIPO_PLANTA_INTERNA, $idEECC, null, null, null));
               ...
               $result = $this->lib_utils->getHTMLPermisos($permisos, ID_PERMISO_PADRE_PLANTA_INTERNA, ID_PERMISO_HIJO_BANDEJA_COTIZACION_PLANTA_INTERNA, ID_MODULO_PAQUETIZADO);
               $data['opciones'] = $result['html'];
        	
        	         $this->load->view('vf_plantaInterna/V_bandeja_cotizacion',$data);
            ...
            foreach($listaPTR->result() as $row){              
                $html .=' 
                        <tr>
                            ...
                            <td><a href="'.(($row->idTipoPlanta==1 ) ? 'detalleObra?item='.$row->itemPlan.'&from=1' : 'detallePI?item='.$row->itemPlan.'&from=1').'" target="_blank">'.$row->itemPlan.'</a></td>
                            ...
                            <th style="text-align: center;'.(($row->hasAdelanto == '1') ? 'color: GREEN;font-size: large' : 'color: currentColor;font-size: initial ').';font-weight: bold;">'.(($row->hasAdelanto == '1') ? 'SI' : 'NO').'</th>
                             <th><a data-idlog ="'.$row->itemPlan.'" onclick="mostrarLog(this)"><img alt="Ver Log" height="20px" width="20px" src="public/img/iconos/lupalog.png"></a></th> 
    ...
    function filtrarTablaCotizacion(){
        ...
        try{
            ...
            $query = $this->m_consulta->getPtrConsulta($itemPlan,'','',$zonal,'',$SubProy,ID_ESTADO_PRE_DISENIO,'',ID_TIPO_PLANTA_INTERNA,$eecc, $idUsuario, $mesEjec);
