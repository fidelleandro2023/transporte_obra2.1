...
class M_registro_po_mo extends CI_Model{
	...
	function   getPartidasByProyectoEstacion($itemplan, $idEstacion){
	    $Query = '(SELECT pa.codigo, pa.descripcion, co.nombre as descPrecio, pa.baremo, pxc.costo  
				...
				UNION ALL
				(
				SELECT pxc.codigo, pa.descripcion, co.nombre as descPrecio, pa.baremo, pxc.costo  
				    FROM planobra po,
	...
	function getInfoCodigoMaterial($itemplan, $idEstacion, $codigo, $costo){
	    $Query = "SELECT pa.idActividad, pa.codigo, pa.descripcion, co.nombre as descPrecio, pa.baremo, pxc.costo  
				...
				SELECT pa.idActividad, pa.codigo, pa.descripcion, co.nombre as descPrecio, pa.baremo, pxc.costo  
				    FROM planobra po,
						 partida_x_zona_x_empresacolab_x_subproyecto pxc,
					     partidas pa,
					     contrato co
				   WHERE po.itemplan = ?
					 ...
					 AND pxc.idSubProyecto  = po.idSubProyecto
				     AND pxc.idZona    = po.idZona" ;
	    $result = $this->db->query($Query,array($itemplan, $codigo, $costo, $itemplan, $codigo, $costo));
	...
	*/
	...
	public function getInfoEstacionByItemplan($itemplan)
    {
		...
		public function getDetallePoManual($itemplan)
    {
        $sql = "   SELECT pxc.id_partida,pxc.codigo, pa.descripcion, co.nombre as descPrecio, pa.baremo, pxc.costo,
			              tb.cantidad,
					      ROUND(pxc.costo*tb.cantidad,2)total	 
				     FROM planobra po,
						  partida_x_zona_x_empresacolab_x_subproyecto pxc,
					      partidas pa,
					      contrato co,
						  tb_kit_mano_obra_manual tb
				    WHERE po.itemplan = ?
				      AND po.idContrato      = co.id_contrato
					  AND po.idEmpresaColab  = pxc.idEmpresaColab 
				      AND pa.idActividad     = pxc.id_partida
					  AND pxc.idSubProyecto  = po.idSubProyecto
				      AND pxc.idZona    = po.idZona
					  AND pxc.codigo = tb.codigo_partida
					  AND pxc.idZona = tb.idZona
					  AND pxc.idEmpresaColab = tb.idEmpresaColab";

        $result = $this->db->query($sql, array($itemplan));
        return $result->result_array();
    }
	
	function updatePoMo($itemplan, $codigoPo, $arrayUpdate) {
	    $data['error'] = EXIT_ERROR;
	    $data['msj']   = null;
	    try{
	        $this->db->trans_begin();	

	        $this->db->where('ptr', $codigoPo);
			$this->db->where('itemplan', $itemplan);
			$this->db->update('ptr_planta_interna', $arrayUpdate);
	        if($this->db->affected_rows() != 1) {
	            $this->db->trans_rollback();
	            throw new Exception('Error al actualizar en ptr_planta_interna');
	        }else{
	            $data['error'] = EXIT_SUCCESS;
				$data['msj'] = 'Se actualizo correctamente!';
				$this->db->trans_commit();
