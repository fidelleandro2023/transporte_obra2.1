...
class M_detalle_obra extends CI_Model {
...
estacion.estacionDesc, area.areaDesc, planobra.fechaPrevEjec AS fec_prev_ejec, '' AS usuario_asig_grafo, CASE WHEN area.tipoArea = 'MAT' THEN '5000.00' ELSE '' END, CASE WHEN area.tipoArea = 'MO' THEN '5000.00' ELSE '' END, '', '', '',
    ...
    public function getDetallePqtIP($itemplan) {
        $sql = " SELECT po.itemPlan,
						c.idCentral,
						po.idSubProyecto,
						s.subProyectoDesc,
				  ...
				  WHERE s.idSubProyecto = po.idSubProyecto
					...
					AND c.idZonal = z.idZonal
					AND po.idEstadoPlan = t.idEstadoPlan
					AND	ec.idEmpresaColab = po.idEmpresaColab
					AND po.itemPlan = '" . $itemplan . "'";
        $result = $this->db->query($sql);
        return $result->row_array();
    }
    ...
    public function getDetallePOPqt($itemplan, $codigoPO, $idEstacion) {
        $sql = "	SELECT tb1.*,
                		   jse.codAlmacen,
                           iea.porcentaje
                      FROM (            
    ...
    function getPartidasBasicByPtrV2_0($ptr) {
        $Query = ' SELECT
                    pa.codigo,
                    pa.descripcion,
                    (CASE WHEN pa.flg_tipo = 3 THEN pqttp.descripcion ELSE pd.descPrecio END) AS descPrecio,
                    pod.costo,
                    pod.baremo,
                    pod.cantidad_inicial,
                    pod.monto_inicial,
                    pod.cantidad_final,
                    pod.monto_final
                    FROM
                    planobra_po_detalle_mo pod,
                    partidas pa
                    LEFT JOIN
                    precio_diseno pd ON pa.idPrecioDiseno = pd.idPrecioDiseno
                    LEFT JOIN
                    pqt_partidas_paquetizadas_x_estacion pqte ON pqte.idActividad = pa.idActividad
                    LEFT JOIN
                    pqt_tipo_preciario pqttp ON pqttp.id = pqte.id_pqt_tipo_preciario
                    
                    WHERE
                    pod.idActividad = pa.idActividad
                    AND pod.codigo_po = ?';
        $result = $this->db->query($Query, array($ptr));
        return $result->result();
    }
    ...
    public function getInfoDetallePlanByCodPo($codigoPO) {

        $sql = "SELECT  poCod, oc as orden_compra, ncert as nro_certificacion , 0 as monto_mo, '' as pep1 from detalleplan where poCod = ? limit 1";
