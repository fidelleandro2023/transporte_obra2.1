...
class M_registro_oc_solicitud_masivo_cert extends CI_Model{
	...
	function getInfoSolicitudOCCreaByCodigoOpex($codigo_solicitud){
	    $Query = "SELECT   po.itemplan, po.idEstadoPlan, po.idSubProyecto, po.paquetizado_fg, sp.idTipoPlanta, soc.codigo_solicitud, soc.estado, count(1) as cant 
...
_log("ENtasdad3333");				
	...
	function masiveUpdateAtencionCreateSolOCOpex($itemplanList, $solicitudes_list) {
	    $data['error'] = EXIT_ERROR;
	    $data['msj']   = null;
	    try{
	        $this->db->trans_begin();
			...
			if($this->db->trans_status() === FALSE) {
				...
				if($this->db->trans_status() === FALSE) {
					$this->db->trans_rollback();
					throw new Exception('Error al modificar el itemplan_solicitud_orden_compra');
				}else{				
					$data['error'] = EXIT_SUCCESS;
					$data['msj'] = 'Se actualizo correctamente!';
					$this->db->trans_commit();    	                          
