...
//concat(substring(year(NOW()),3,2),'-',
            $query = "SELECT concat(" . ANIO_CREATE_ITEMPLAN . ",'-',
						   (CASE WHEN LENGTH(" . $idProy . ") = 1 THEN CONCAT('0'," . $idProy . ") ELSE ROUND((RAND() * (99 - 10)) + 10) END),
							(SELECT case when (SELECT substring(zonalDesc,1,4) 
					  FROM zonal 
					 WHERE idzonal =" . $idzonal . ")='LIMA' THEN '1' else '2' end),
							CASE WHEN LENGTH(" . $idzonal . ") = 1 then CONCAT('0'," . $idzonal . ") ELSE ROUND((RAND() * (99 - 10)) + 10) END) AS itemplan;";
            $result = $this->db->query($query, array());
