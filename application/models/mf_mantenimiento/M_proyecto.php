...
class M_proyecto extends CI_Model{
	...
	function insertProyecto($decripcion, $tipoCentral, $tipoLabel, $idGerenciaProyecto){
	...
	function editarProyecto($id, $idTipocentral, $tipoLabel, $descripcion, $idGerencia){
	    $data['error'] = EXIT_ERROR;
	    $data['msj']   = null;
	    try{
	        $this->db->trans_begin();
...
// 	                $listaEstacionAreas = explode(',', $stringEstaciones);
	...
	function updateNewSubProyectoDespliegue($dataUpdate, $estacionesArea, $idUsuario, $flgDelete)
	{
	    $data['error'] = EXIT_ERROR;
	    $data['msj']   = null;
	    try{
			$idSubProyectoGlobal = $dataUpdate['idSubProyecto'];
	        $this->db->trans_begin();	
	        $this->db->where('idSubProyecto', $idSubProyectoGlobal);
	        $this->db->update('subproyecto', $dataUpdate);
			if ($this->db->trans_status() === FALSE) {	          
	            $this->db->trans_rollback();
	            throw new Exception('Error al actualizar subproyecto');
	        }else{
				if($flgDelete == 2){
					$this->db->where('idSubProyecto', $idSubProyectoGlobal);	               
					$this->db->delete('subproyectoestacion');
					if ($this->db->trans_status() === FALSE) {
						$this->db->trans_rollback();
						throw new Exception('Hubo un error al eliminar en subproyectoestacion.');
					}else{	
						$insert_id = $idSubProyectoGlobal;	            
						$subproyectoEstacionList = array();
						foreach($estacionesArea as $estacionArea){
							$subEsta = array();
							$subEsta['idSubProyecto']	=	$insert_id;
						...
						if ($this->db->trans_status() === FALSE) {
							...
							$logSubPro = array('idSubProyecto'		=>	$insert_id,
												'idUsuario'			=>	$idUsuario,
												'fecha_registro'	=>	$this->fechaActual(),
												'actividad'			=>	'Actualiza SuProyecto');
	...
	function insertUpdateFaseCantPlanificado($dataInsert, $dataUpdate)
	{
	    $data['error'] = EXIT_ERROR;
	    $data['msj']   = null;
	    try{
	        $this->db->insert_batch('subproyecto_fases_cant_itemplan', $dataInsert);
			if ($this->db->trans_status() === FALSE) {
				$this->db->trans_rollback();
				throw new Exception('Hubo un error al insertar el subproyectoestacion.');
			}else{	
				if(count($dataUpdate) > 0){
					foreach($dataUpdate as $row){
						...
						$this->db->where('fase', $row['fase']);
						$this->db->update('subproyecto_fases_cant_itemplan', $dataUpdate);
						if ($this->db->trans_status() === FALSE) {
							$this->db->trans_rollback();
							throw new Exception('Hubo un error al actualizar en subproyecto_fases_cant_itemplan.');
						}else{
							$this->db->trans_commit();
							$data['error']  = EXIT_SUCCESS;
							$data['msj'] = 'Se actualizo correctamente!';
	...
	function getCountSubProyFaseCant($idSubProyecto,$fase)
	{
		 $sql = " SELECT COUNT(*) AS cantidad
		            FROM subproyecto_fases_cant_itemplan
				   WHERE idSubProyecto = ?
				     AND fase = ? ";
        $result = $this->db->query($sql, array($idSubProyecto,$fase));
