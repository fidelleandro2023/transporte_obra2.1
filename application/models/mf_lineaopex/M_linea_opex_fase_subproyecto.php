<?php
class M_linea_opex_fase_subproyecto extends CI_Model{
    ...
    {
        $sql = "SELECT * FROM subproyecto_lineaopex_combinatoria";//estado = 1 : activo
    ...
    public function getcombinatoria() {
        $sql = "SELECT * FROM combinatoria_opex";
    ...
    public function getlineaxfase() {
        $sql = "SELECT DISTINCT * FROM (SELECT fa.faseDesc,li.idlineaopex,li.`idlineaopex_fase`,li.presupuesto FROM lineaopex_fase as li INNER JOIN fase as fa ON li.idfase = fa.idFase) as que INNER JOIN lineaopex as fa ON que.idlineaopex = fa.idlineaopex";

        $result = $this->db->query($sql);
        return $result->result();
    }

    public function getlineaxfasesubproyecto() {
        $sql = "SELECT * FROM subproyecto_lineaopex_combinatoria";
    ...
    function saveConfigOpexLinea($dataInsert,$linea,$subproyecto) {
        $data['error'] = EXIT_ERROR;
        $data['msj'] = null;
        try {

            $this->db->trans_begin();
            $this->db->where('idSubProyecto',$subproyecto);
            $this->db->where('idlineaopex_fase',$linea);
            
            $query = $this->db->get('subproyecto_lineaopex_combinatoria');
            if ($query->num_rows() > 0){
                $data['error'] = EXIT_ERROR;
                $data['msj'] = 'Ya se encuentra asociada la Linea Opex a la Fase seleccionada';
            }else{
         
            $this->db->insert('subproyecto_lineaopex_combinatoria', $dataInsert);
