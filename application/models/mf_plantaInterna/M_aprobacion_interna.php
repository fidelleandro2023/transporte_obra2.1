<?php
class M_aprobacion_interna extends CI_Model{
    ...
    function getPtrToLiquidacion($subPry, $eecc, $zonal, $itemPlan, $mesEjec, $area, $estado, $idUsuario, $estadoPtr){
        ...
        $Query = "   SELECT pp.ptr, 
                            pp.flg_rechazado,
                            pp.itemplan, sp.subProyectoDesc, z.zonalDesc, ec.empresaColabDesc, a.areaDesc, substring(pp.estado,1,2) as estado,
                    CASE
                    ...
                    po.fechaPrevEjec,
                    SUM(ptraz.costo_mo) as costo_mo, 
                    SUM(ptraz.costo_mat) as costo_mat, 
                    SUM(ptraz.total) as total,
                    2 as tipo_po,
					po.orden_compra,
					po.solicitud_oc
        ...
        if($zonal!=''){
            $Query .= " AND z.zonalDesc REGEXP '".str_replace(',','|',$zonal)."'";
        }

        if($area!=''){
            $Query .=  " AND a.areaDesc = '".$area."'";
        }

        if($mesEjec!=NULL){
            $Query .=  " AND EXTRACT(MONTH FROM DATE(po.fechaPrevEjec)) = $mesEjec";
		}
        // if($mesEjec!=''){
        ...
        if($idUsuario!=NULL) {
			$Query .=  " AND po.itemplan IN ( SELECT l.itemplan
											   FROM log_planobra l
            ...
            UNION ALL
			 SELECT pp.codigo_po, 
               ...
               WHERE po.itemplan = pp.itemplan
				AND	 pp.itemplan              = dp.itemplan
                 AND pp.codigo_po	= dp.poCod
...
and ptr_expediente.ptr is null) as hasDise,
	...
	function aprobarCotizacionPI($dataPlanobra, $solicitud_oc_creacion, $item_x_sol) {
	    $data['error'] = EXIT_ERROR;
	    $data['msj'] = null;
	    try {	
	        $this->db->trans_begin();
    ...
    ///itemplan_solicitud_orden_compra tabla para opex
	function aprobarCotizacionPIOpex($dataPlanobra, $solicitud_oc_creacion, $item_x_sol) {
	    $data['error'] = EXIT_ERROR;
	    $data['msj'] = null;
	    try {	
	        $this->db->trans_begin();
            ...
            if ($this->db->trans_status() === FALSE) {
                ...
                $this->db->insert('itemplan_solicitud_orden_compra', $solicitud_oc_creacion);
                if ($this->db->affected_rows() != 1) {
                    $this->db->trans_rollback();
                    throw new Exception('Error al insertar en solicitud_orden_compra');
                } else {
	...
	function getListadoItemplan() {
        $sql = " SELECT * FROM tb_listado_itemplan_oc ";
        $result = $this->db->query($sql);
