<?php
class M_detalle_planta_interna extends CI_Model{
	...
	function   getAllEstaciones($item){
	    $Query = "  SELECT planobra.itemPlan, estacion.estacionDesc,  estacion.idEstacion, area.areaDesc, subproyectoestacion.idSubProyectoEstacion 
	...
	function   getAllAreasByEstacion_old($item, $estacion){
	    $Query = "SELECT planobra.itemPlan, 
		                 subproyectoestacion.idSubProyectoEstacion, 
						 estacion.idEstacion, estacion.estacionDesc, 
						 area.idArea, 
						 area.areaDesc, 
						 area.tipoArea, 
						 planobra.idEstadoPlan,
	...
	function getAllPTRbyArea($item, $estacion, $area){
		...
		$Query = "  SELECT planobra.itemPlan, subproyectoestacion.idSubProyectoEstacion, estacion.estacionDesc, area.areaDesc, ptr_planta_interna.ptr, ptr_planta_interna.rangoPtr, estacion.idEstacion
					FROM planobra, subproyectoestacion, estacionarea, estacion, area, ptr_planta_interna 
					WHERE planobra.itemPlan = ptr_planta_interna.itemPlan
					and ptr_planta_interna.idSubProyectoEstacion = subproyectoestacion.idSubProyectoEstacion
					AND subproyectoestacion.idEstacionArea = estacionarea.idEstacionArea
					AND estacionarea.idEstacion = estacion.idEstacion
					AND estacionarea.idArea = area.idArea
					AND planobra.flg_transporte IS NULL
					...
					and estacion.estacionDesc = ?
					and area.areaDesc = ?
					ORDER BY estacion.estacionDesc, area.areaDesc, ptr_planta_interna.ptr" ;
	...
	function getAllPTRbyAreaMAT($item, $estacion, $area){
	    ...
	    $Query = "  SELECT planobra.itemPlan, subproyectoestacion.idSubProyectoEstacion, estacion.estacionDesc, area.areaDesc, detalleplan.poCod as ptr, estacion.idEstacion,
                    	...
                    	FROM planobra, subproyectoestacion, estacionarea, estacion, area, detalleplan, planobra_po
                    	WHERE planobra.itemPlan = planobra_po.itemplan
    ...
    function getInfoPtrInterna($ptr){
		$Query = "  SELECT pp.ptr, pp.vale_reserva, pp.itemplan, sp.subProyectoDesc, z.zonalDesc, ec.empresaColabDesc, a.areaDesc,  pp.estado, c.jefatura,
pp.fecha_crea, pp.fecha_aprob, pp.usua_aprob,
 ROUND(SUM(ptraz.costo_mo),2) as costo_mo, ROUND(SUM(ptraz.costo_mat),2) as costo_mat, pp.rangoPtr, a.tipoArea, po.idEstadoPlan
                FROM 
                    ...
                    AND pp.ptr = ptraz.ptr
                    AND pp.ptr = ?
                    AND (po.paquetizado_fg = 2 OR po.paquetizado_fg = 1)
					AND po.flg_transporte IS NULL
                    GROUP BY ptr                     
                    UNION ALL
                    SELECT pp.ptr, pp.vale_reserva, pp.itemplan, sp.subProyectoDesc, z.zonalDesc, ec.empresaColabDesc, a.areaDesc,  pp.estado, c.jefatura,
					pp.fecha_crea, pp.fecha_aprob, pp.usua_aprob,
					ROUND(SUM(ptraz.costo_mo),2) as costo_mo, 
					ROUND(SUM(ptraz.costo_mat),2) as costo_mat, pp.rangoPtr, a.tipoArea, po.idEstadoPlan
									FROM 
                    ptr_planta_interna pp, 
                    planobra po, 
                    empresacolab ec,
					...
					AND po.flg_transporte IS NULL
                    GROUP BY ptr  " ;
	    $result = $this->db->query($Query,array($ptr, $ptr));
           if($result->row() != null) {
...
estacion.estacionDesc, area.areaDesc, planobra.fechaPrevEjec AS fec_prev_ejec, '' AS usuario_asig_grafo, CASE WHEN area.tipoArea = 'MAT' THEN '5000.00' ELSE '' END, CASE WHEN area.tipoArea = 'MO' THEN '5000.00' ELSE '' END, '', '', ''
	...
	/**nuevo czavalacas 16.07.2019 ****/

	function getCountPtrPlantaInterna($itemplan, $idEstacion, $tipoArea) {
	    $sql = "SELECT COUNT(1) count
                FROM 	ptr_planta_interna ptr, subproyectoestacion se, estacionarea ea, area a
