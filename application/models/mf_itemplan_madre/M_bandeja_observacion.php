...
class M_bandeja_observacion extends CI_Model {
	...
	COUNT(*) total
        FROM
	planobra po
	LEFT JOIN subproyecto sb ON po.idSubProyecto = sb.idSubProyecto
	LEFT JOIN proyecto pr ON sb.idProyecto = pr.idProyecto
	LEFT JOIN empresacolab em ON em.idEmpresaColab = po.idEmpresaColab
	LEFT JOIN estadoplan est ON est.idEstadoPlan = po.idEstadoPlan
	...
	AND ( ( CASE WHEN vali.estado_di THEN vali.estado_di ELSE 0 END ) + ( CASE WHEN vali.estado_ope THEN vali.estado_ope ELSE 0 END ) ) IN(3,5,6) 
...
ORDER BY
	...
	po.itemPlanPE = ? AND po.idEstadoPlan=7
	AND vali.estado_di = 2 OR vali.estado_ope = 2;';
