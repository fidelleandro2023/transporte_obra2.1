...
//    }
    ...
    function getAllSubProyecto($idTipoPlantaInterna = null) {//SE QUITO 2017 POR PEDIDO DE OWEN EN PRESUPUESTO 07.02.2019
        $Query = "SELECT * 
	                FROM subproyecto
    ...
    function getAllCentral() {
        ...
        $result = $this->db->query($Query, array());
		log_message('error','test getAllCentral');
        return $result;
    ...
    function getAllCentralPqt($idEmpresaColab = NULL) {
        $Query = "  SELECT idCentral ,
		              ...
		              FROM pqt_central
					 WHERE /*idTipoCentral IN (1,2) AND*/
					    CASE WHEN ? IN (0,6) THEN true 
	...
	function getAllProyectoPI() {
        $Query = "SELECT * 
		            FROM proyecto
                   WHERE idProyecto IN ( SELECT subp.idproyecto
                                           FROM subproyecto subp
                                          #WHERE subp.idTipoPlanta = 2
	...
	*/
    ...
    function getAllMotivos() {
        $Query = "SELECT * from motivo order by motivoDesc";
        $result = $this->db->query($Query, array());
        return $result;
    }
    ...
    function getAllSubProyectoPaquetizadoDesc() {
        $Query = "  SELECT     subproyecto.idSubProyecto,
	                           ...
	                           proyecto.proyectoDesc,
	                           subproyecto.subProyectoDesc,
	                           tipoplanta_transporte.tipoPlantadesc
	                   FROM    subproyecto, proyecto, tipoplanta_transporte
            	       WHERE   subproyecto.idProyecto = proyecto.idProyecto
            	       AND     subproyecto.idTipoPlanta = tipoplanta_transporte.idTipoPlanta
            	    ORDER BY   proyectoDesc, subProyectoDesc;";
        $result = $this->db->query($Query, array());
        return $result;
    }
...
// 	    log_message('error', 'insertarLogPqtCentral $arrayInsert:'.print_r($arrayInsert,true));
    ...
    function canCreatePoMat($itemplan) {#VALIDACION PEDIDO DE OWEN 13.01.2020 no crear po mat obras menores al 17 nov 2019
        $sql = "SELECT  count(1) as cant
	            FROM    planobra 
    ...
    public function itemplanExecptionCreatePoMat($itemplan) {#VALIDACION PEDIDO DE OWEN 13.01.2020 no crear po mat obras menores al 17 nov 2019
...
'20-0321500082')";
    ...
    function getNextCodSolicitud() {//06.07.2020
	...
	function getDataPoByItemplan($itemplan) {
		$sql = " SELECT ptr, 
                        ...
                        ROUND(SUM(total), 2) costo_mo,
                        po.itemplan,
                        po.costo_unitario_mo_crea_oc,
                        CASE WHEN ROUND(SUM(total), 2) <> po.costo_unitario_mo THEN 1 
	...
	function getInfoItemplanWhitSubProyecto($itemplan) {
	    $Query = "select po.itemplan, 
						 ...
						 po.idEmpresaColab
				from planobra po, subproyecto sp where po.idSubProyecto = sp.idSubProyecto
				and po.itemplan = ?";
	    $result = $this->db->query($Query, array($itemplan));
	    if ($result->row() != null) {
	        return $result->row_array();
	...
	public function removeOTAC($itemplan, $codigo_ot, $log_planobra) {
	    ...
	    try {
	        ...
	        if ($this->db->trans_status() === true) {
	            $this->db->where('itemPlan', $itemplan);
	            $this->db->update('planobra', array('utilmo_codigo_sirope_ac' => null,
	...
	function generarSolicitudCertiAnulEdiOC($itemplan, $nom_plan, $tipo_solicitud, $costo_edit_certi = null) {
        $sql = "SELECT fn_create_solicitud_certi_oc(?, ?, ?, ?) AS flgValida";
        $result = $this->db->query($sql, array($itemplan, $nom_plan, $tipo_solicitud, $costo_edit_certi));
        return $result->row_array()['flgValida'];
    }
	...
	function getDetallePoTransp($codigo_po, $itemplan = null) {
		$sql = "  SELECT ppd.ptr as codigo_po,
			   ...
			   LEFT JOIN partidas pa 
			          ON pa.idActividad = ppd.id_actividad
				   WHERE ptr = COALESCE(?, ptr)
				     AND itemplan = COALESCE(?, itemplan)";
		$result = $this->db->query($sql, array($codigo_po, $itemplan));
		return $result->result_array();
	}
    ...
    function getAllEmpresaColabByIdContratoPadre($idContratoPadre = null) {
        $sql = "   SELECT *
                    ...
                    WHERE ce.id_contrato_padre = COALESCE(?, ce.id_contrato_padre)
                      AND e.idEmpresaColab = ce.idEmpresaColab
                 ORDER BY empresaColabDesc, ce.id_contrato_padre";
	...
	function getEstacionDBSAM() {
        $sql = " SELECT * FROM sam_db.estacion LIMIT 1 ";
        $result = $this->db->query($sql, array());
        return $result->result_array();
    }
